...
Celu ovu logiku najlakše je razumeti sa slike. Slika 45.1 prikazuje sve momentne karakteristike iz zadatka u jednom dijagramu — obe karakteristike motora, njihove linearizacije, obe karakteristike tereta i radne tačke 1, 2 i 3; detaljno čitanje dato je odmah ispod slike.
...
> **Kako čitati sliku 45.1:** Ose: na horizontalnoj je klizanje $s$ (bezdimenziono, od $0$ do $1$, podeoci na po $0{,}1$; pazi — $s = 0$ je sinhrona brzina, $s = 1$ ukočen rotor, dakle **brzina raste ulevo**), na vertikalnoj moment normalizovan na nazivni, $M/M_n$ (od $0$ do $4$). Krive i prave, po boji i oznaci sa slike: **ružičasta (magenta) kriva** ($R_r$) — prirodna karakteristika motora, sa prevalnim momentom $M_{pr} \approx 2\,M_n$ na prevalnom klizanju $s_{pr} \approx 0{,}2$ (obe veličine objašnjava mini-lekcija 4); **crvena strma prava** ($2 \cdot M_{pr} \cdot s/s_{pr}$) — linearizacija njenog radnog dela, prava kroz koordinatni početak koja se s krivom poklapa samo za $s \ll s_{pr}$; **svetloplava (cijan) kriva** ($R_r + R_{rd}$) — karakteristika sa uključenim dodatnim otporom: njeno prevalno klizanje je $13$ puta veće ($s'_{pr} \approx 2{,}6$, daleko van desne ivice slike), pa u celom opsegu $0 \le s \le 1$ raste gotovo linearno; **zelena prava** ($2 \cdot M_{pr} \cdot s/s'_{pr}$) — njena linearizacija, koja se sa cijan krivom na slici praktično poklapa; **siva isprekidana horizontala** ($M_{t2} = M_t$, na visini $M/M_n \approx 1$) — zamišljeni konstantni teret iz pomoćnog Koraka 2; **siva puna opadajuća prava** ($M_t = f(s)$) — stvarni teret $M_t = 0{,}1 \cdot n = 0{,}1 \cdot (1-s) \cdot n_s$, koji od najveće vrednosti pri $s = 0$ linearno pada na nulu pri $s = 1$ (rotor tada stoji, $n = 0$). Radne tačke (crni kružići sa strelicama ka apscisi): **1** — presek prirodne karakteristike (i njene crvene linearizacije) sa teretom, na $s_1 = 0{,}05$, visina $M_1 = 285\ \mathrm{Nm}$ (na slici $\approx 1$; oznaka $M_{t1}$); **2** — presek nove karakteristike sa isprekidanom horizontalom konstantnog tereta, na $s_2 = 0{,}65$; **3** — traženi presek nove karakteristike sa stvarnim (opadajućim) teretom, na $s_3 = 0{,}406$, visina $M_3 = 178\ \mathrm{Nm}$, tj. $M_3/M_1 \approx 0{,}62$ (oznaka $M_{t3}$); tačka 3 leži levo i niže od tačke 2, jer teret pri manjoj brzini traži manji moment. **Šta treba da zaključiš:** dodatni rotorski otpor ne menja visinu "brda" momenta, nego karakteristiku razvlači udesno — radna tačka zato sklizne na veće klizanje, ali se kod tereta koji opada sa brzinom ravnoteža uspostavi već na $s_3 = 0{,}406$, a ne na $s_2 = 0{,}65$.
