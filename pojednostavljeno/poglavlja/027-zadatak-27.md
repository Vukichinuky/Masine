...
Sledeća slika prikazuje blok-šemu takvog pogona. Čitaj je sleva nadesno: spoljašnji regulatori (fluksa — blok 1, i brzine — blok 9) proizvode reference struja $i_{sd}^{REF}$ i $i_{sq}^{REF}$; dva odvojena strujna regulatora (blok 2) upoređuju reference sa merenim $i_{sd}$, $i_{sq}$ i zadaju napone $u_{sd}$, $u_{sq}$ (blok 3, "RB", je rasprežući blok koji kompenzuje ukrštene uticaje osa); transformacija $e^{j\theta_s}$ (blok 4) prebacuje napone iz rotirajućeg dq sistema u nepokretni sistem, naponski modulator SPWM (blok 5; sinusna širinsko-impulsna modulacija) pravi impulse za tranzistore invertora, a invertor napaja motor. Povratna grana ide zdesna nalevo: merene fazne struje $i_{sa}, i_{sb}, i_{sc}$ se transformacijom "3/2" (blok 6) i rotacijom $e^{-j\theta_s}$ (blok 7) svode na $i_{sd}$, $i_{sq}$; blok 8 ("FM", model fluksa) daje amplitudu i položaj fluksa, a davač na vratilu daje brzinu $\omega_r$ i položaj rotora.
...
Situaciju prikazuje sledeći fazorski dijagram. Kako da ga čitaš: d-osa i q-osa su ose rotirajućeg koordinatnog sistema; struja $i = i_q$ leži na q-osi; na nju se nadovezuju ems $e$ i omski pad $r_s i$ (oba duž q-ose), a pad na reaktansi $x_q i_q$ ide **normalno** na struju (ka negativnoj d-strani, jer je $u_d$ negativno); vektor $u$ je hipotenuza — zbir svega. Ugao $\varphi$ između $u$ i $i$ je ugao faktora snage. **Pažnja pri čitanju slike:** autor zbirke je vektor fluksa rotora $\psi_f$ nacrtao na **negativnoj** strani d-ose (strelica ulevo, dok oznaka ose pokazuje udesno). Suštinu to ne menja — bitno je da je $\psi_f$ u d-pravcu, normalan na struju.
...
Novu situaciju prikazuje sledeći fazorski dijagram. Kako da ga čitaš: vektor struje $i$ (isprekidana linija pokazuje njegov pravac) otklonjen je od q-ose za ugao $\gamma$; njegove projekcije su $i_q$ na q-osi i $i_d$ na d-pravcu — nacrtana **suprotno** od strelice $\psi_f$, što je grafički prikaz činjenice da je d-komponenta demagnetišuća (u našem zapisu: $i_d < 0$). Napon $u$ se i dalje sastavlja od ems $e$, omskog pada $r_s i$ (paralelnog struji), i padova na reaktansama: segment $x_d i_d$ leži na q-pravcu (jer d-struja indukuje napon u q-osi), a $x_q i_q$ na d-pravcu. Ugao $\varphi$ je i dalje ugao između $u$ i $i$. Kao i na slici 27.2, autor je $\psi_f$ nacrtao na negativnoj strani ose označene sa d — bitan je samo međusobni odnos: $i_d$ i $\psi_f$ su suprotnog smera.
