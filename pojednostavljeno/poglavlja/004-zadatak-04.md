...
Jednačina $\overline{E}_{0f} = \overline{U}_{\mathrm{f}} + R_s\overline{I}_f + \mathrm{j}X_s\overline{I}_f$ je sabiranje vektora, i najlakše se „vidi" na dijagramu — prikazuje ga slika 4.1 (ispod).
...
> **Kako čitati sliku 4.1:** Fazorski (vektorski) dijagram bez brojčanih osa — dužine vektora su srazmerne voltima, a uglovi se čitaju direktno sa slike; fazori rotiraju suprotno kazaljci na satu, pa vektor zakrenut od reference suprotno kazaljci *prednjači*, a u smeru kazaljke *kasni*. Iz zajedničkog početka (dole) polaze tri vektora. Referenca je fazni napon mreže $\underline{U}_f$, nacrtan uspravno ($U_f = 219{,}39\ \mathrm{V}$ u našem zadatku). Vektor struje $\underline{I}_f$ (kraći, desno od napona) **kasni** za naponom za ugao $\varphi$ — donji desni luk; u starom režimu $\varphi = 36{,}87^\circ$ (generator daje induktivnu reaktivnu snagu). Vektor $\underline{E}_{0f}$ (najduži, gore levo; $E_{0f} = 339{,}7\ \mathrm{V}$) **prednjači** naponu za ugao snage $\delta$ — donji levi luk. Konstrukcija zbira ide „vrh na rep": na vrh $\underline{U}_f$ nadovezan je mali pad $R_s\underline{I}$, paralelan struji (pad na otporniku je u fazi sa strujom; $R_s I_f = 1{,}5\cdot 7{,}6 = 11{,}4\ \mathrm{V}$ — zato je jedva vidljiv), a na njega dugi pad $\mathrm{j}X_S\underline{I}_f$, normalan na pravac struje (množenje sa $\mathrm{j}$ = zaokret za $+90°$; normalnost je označena pravim uglom sa tačkom; $X_s I_f = 20\cdot 7{,}6 = 152\ \mathrm{V}$); kraj te konstrukcije pogađa tačno vrh $\underline{E}_{0f}$ — to je grafički zapis jednačine $\overline{E}_{0f} = \overline{U}_f + R_s\overline{I}_f + \mathrm{j}X_s\overline{I}_f$. Tačkasta linija od vrha $\underline{U}_f$ do vrha $\underline{E}_{0f}$ je ukupni pad napona $Z\,\underline{I}_f = 152{,}46\ \mathrm{V}$, a mali ugao $\alpha$ pri samom vrhu, između pada $\mathrm{j}X_S\underline{I}_f$ i te tačkaste linije, jeste dopuna ugla impedanse do $90°$: $\alpha = \operatorname{arctg}(R_s/X_s) = 4{,}29°$. Šta treba da zaključiš: trougao sa stranicama $U_f$, $Z I_f$ i $E_{0f}$ — sa tupim uglom $\gamma = 90° + \alpha + \varphi = 131{,}16°$ između prve dve — upravo je trougao na koji u Koraku 2 primenjujemo kosinusnu teoremu; a pošto se sa slike vidi $E_{0f} > U_f$, generator je nadpobuđen.
