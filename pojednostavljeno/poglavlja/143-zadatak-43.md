...
Sledeća slika prikazuje mehaničku (momentnu) karakteristiku $M(n)$ za tri vrednosti dodatnog rotorskog otpora iz ovog zadatka.
...
> **Kako čitati sliku 43.1:** Na horizontalnoj osi je brzina obrtanja $n$ u $\mathrm{min^{-1}}$ (od $0$ do $1500$), na vertikalnoj razvijeni moment $M$ u $\mathrm{Nm}$ (od $0$ do $90$); naslov slike je "Razvijeni moment", a plava tačkasta mreža služi za očitavanje. Tri krive razlikuju se bojom i oznakom — pažnja: oznake "$R'_r$" na slici su vrednosti **dodatnog** otpora svedenog na stator, $R'_d$ (v. napomenu ispod slike 43.2): **crvena** kriva ($R'_d = 0$) je prirodna karakteristika — polazni moment $\approx 63\ \mathrm{Nm}$ raste do vrha na $\approx 822\ \mathrm{min^{-1}}$ (slučaj c), pa strmo pada; **zelena** ($R'_d = 0{,}818\ \Omega$) ima vrh tačno na $600\ \mathrm{min^{-1}}$ (slučaj b), a u polasku daje $\approx 74\ \mathrm{Nm}$; **ljubičasta** ($R'_d = 3{,}029\ \Omega$) ima vrh baš na $n = 0$ (slučaj a — najveći mogući polazni moment) i odatle monotono opada. Ključno zapažanje: **sve tri krive imaju istu visinu vrha**, $M_{pr} = 83{,}13\ \mathrm{Nm}$, i sve tri se spuštaju u nulu na sinhronoj brzini $n_s = 1500\ \mathrm{min^{-1}}$ — pri $s = 0$ obrtno polje više ništa ne indukuje u rotoru, pa momenta nema. **Šta treba da zaključiš:** dodatni rotorski otpor ne menja visinu "brda" momenta, nego samo brzinu na kojoj brdo stoji — upravo osobinu koju koristimo u delovima a), b) i c).

Druga slika prikazuje statorsku struju $I_s(n)$ za ista tri slučaja, istim bojama.
...
> **Kako čitati sliku 43.2:** Na horizontalnoj osi je brzina $n$ u $\mathrm{min^{-1}}$ (od $0$ do $1500$), na vertikalnoj statorska struja $I_s$ u $\mathrm{A}$ (od $0$ do $40$); naslov slike je "Statorska struja". Boje su iste kao na slici 43.1: **crvena** — bez dodatnog otpora, polazna struja $\approx 36\ \mathrm{A}$; **zelena** — $R'_d = 0{,}818\ \Omega$, polazna $\approx 34\ \mathrm{A}$; **ljubičasta** — $R'_d = 3{,}029\ \Omega$, polazna $\approx 28\ \mathrm{A}$. Sve tri krive monotono opadaju i stižu u nulu na $1500\ \mathrm{min^{-1}}$ (pri $s = 0$ rotorske struje nema, a struja magnećenja je u ovom modelu zanemarena, $L_m \to \infty$); pri svakoj brzini veći dodatni otpor daje manju struju — zato rotorski otpornik služi i za ograničenje polazne struje. Debela siva horizontala je nivo prevalne struje $I_{s,pr} = 28{,}06\ \mathrm{A}$; crne tačke **a** (na ljubičastoj, pri $n = 0$), **b** (na zelenoj, pri $600\ \mathrm{min^{-1}}$) i **c** (na crvenoj, pri $822\ \mathrm{min^{-1}}$) pokazuju da svaka kriva seče taj nivo tačno na svojoj prevalnoj brzini. **Šta treba da zaključiš:** ma gde dodatni otpor "namestio" prevalnu tačku, struja u njoj je uvek istih $28{,}06\ \mathrm{A}$ — grafička potvrda računa iz Koraka 6.
