...
Pogledajmo sada kako oba režima izgledaju na vektorskom (fazorskom) dijagramu. Na slici koja sledi prikazan je **potpobuđen** motor (prvi režim); detaljno čitanje dato je odmah ispod slike.
...
> **Kako čitati sliku 14.1:** Referentni fazor je napon mreže $\bar{U}_{sf}$ — zeleni horizontalni fazor (u računu $220\ \mathrm{V}$, na realnoj osi); svi uglovi se mere od njega, a rotacija fazora je suprotna kazaljci na satu, pa fazor zakrenut **ispod** realne ose kasni za naponom. Tirkizni fazor ispod ose je motorska struja $-\bar{I}_{sf}'$: kasni za naponom za ugao $\varphi'$ ($\cos\varphi' = 0{,}5$, tj. $\varphi' = 60^\circ$) — induktivni režim; njene komponente su $150\ \mathrm{A}$ (aktivna, u pravcu napona) i $259{,}8\ \mathrm{A}$ (reaktivna, naniže). Crni fazor je EMS $\bar{E}_{0sf}' = (168{,}0 - j\cdot 30)\ \mathrm{V}$: **kraći** od napona ($170{,}7 < 220\ \mathrm{V}$ — potpobuđenost) i zakrenut iza napona za ugao opterećenja $\delta' = 10{,}1^\circ$; na slici ugao $\varphi'$ (do struje) vidljivo je veći od ugla $\delta'$ (do EMS). Zeleni fazor $j\cdot\bar{I}_{sf}'\cdot X_s$, dužine $I'X_s = 300 \cdot 0{,}2 = 60\ \mathrm{V}$, vodi od vrha napona do vrha EMS i time grafički zatvara naponsku jednačinu $\bar{E}_{0sf}' = \bar{U}_{sf} + j\bar{I}_{sf}'X_s$; normalan je na pravac struje (donja oznaka $\pi/2$ sa isprekidanom plavom konstrukcijom). Narandžasti fazor $\bar{F}_r'$ je magnetopobudna sila rotora: prednjači svojoj EMS za $\pi/2$ (gornja oznaka), jer indukovana EMS uvek kasni $90^\circ$ za fluksom koji je stvara. Crveno uokvirene veličine na isprekidanim linijama su dve konstante režima (Mini-lekcija 5): vertikala kroz vrh struje ($-\bar{I}_{sf}'\cdot\cos\varphi' = 150\ \mathrm{A} = \mathrm{konst.}$, kotirano strelicom dole levo) i horizontala kroz vrh EMS ($\bar{E}_{0sf}'\cdot\sin\delta' = 30\ \mathrm{V} = \mathrm{konst.}$, kotirano dvostrukom strelicom desno). **Šta treba da zaključiš:** pri promeni pobude (uz isti teret) vrh struje sme da klizi samo po vertikalnoj, a vrh EMS samo po horizontalnoj isprekidanoj liniji — to je geometrijski "šablon" po kome ćemo konstruisati drugi režim.

Sledeća slika prikazuje isti motor, sa istim teretom, ali **nadpobuđen** (drugi režim); detaljno čitanje dato je odmah ispod slike.
...
> **Kako čitati sliku 14.2:** Isti tip dijagrama za drugi (nadpobuđeni) režim; referenca (zeleni horizontalni $\bar{U}_{sf}$ na realnoj osi) i način čitanja uglova isti su kao na slici 14.1. Tri ključne razlike: tirkizna struja $-\bar{I}_{sf}'' = (150 + j\cdot 87{,}0)\ \mathrm{A}$ sada je **iznad** ose — prednjači naponu za ugao $\varphi''$ ($\cos\varphi'' = 0{,}865$, $\varphi'' \approx 30^\circ$) — kapacitivni režim, sa ukupnom strujom od svega $173{,}4\ \mathrm{A}$ umesto $300\ \mathrm{A}$; crna EMS $\bar{E}_{0sf}'' = (237{,}4 - j\cdot 30)\ \mathrm{V}$ je sada **duža** od napona ($239{,}3 > 220\ \mathrm{V}$ — nadpobuđenost), zakrenuta za ugao opterećenja $\delta'' = 7{,}2^\circ$ iza napona; narandžasta magnetopobudna sila $\bar{F}_r''$ duža je nego na slici 14.1 (pobuda je povećana sa $100$ na $140{,}2\ \mathrm{A}$), i dalje $\pi/2$ ispred EMS. Zeleni fazor $j\cdot\bar{I}_{sf}''\cdot X_s$, dužine $173{,}4 \cdot 0{,}2 = 34{,}7\ \mathrm{V}$, ponovo vodi od vrha napona do vrha EMS i normalan je na struju (isprekidana plava konstrukcija sa oznakom $\pi/2$). Vrh struje leži na **istoj** crvenoj vertikali kao u prvom režimu ($-\bar{I}_{sf}''\cdot\cos\varphi'' = 150\ \mathrm{A} = \mathrm{konst.}$), a vrh EMS na **istoj** crvenoj horizontali ($30\ \mathrm{V}$ ispod ose); u žutom okviru dole desno na slici piše $\sin(\theta'')$ — štamparska omaška u originalnoj slici, treba $\sin(\delta'')$, isto kao na slici 14.1. **Šta treba da zaključiš:** pošto se aktivno opterećenje nije promenilo, obe isprekidane linije ostale su na svom mestu — pojačana pobuda samo je "prešetala" vrh struje po vertikali (iz kašnjenja u prednjačenje) i vrh EMS po horizontali (iz $E_{0sf} < U_{sf}$ u $E_{0sf} > U_{sf}$).
