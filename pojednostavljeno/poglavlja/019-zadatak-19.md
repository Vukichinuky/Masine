...
Slika 19.1 prikazuje ovaj dijagram za naš režim.
...
> **Kako čitati sliku 19.1:** Fazorski dijagram u dq-koordinatama vezanim za rotor. Vertikalna osa (naviše) je **q-osa**, horizontalna (udesno) **d-osa**; podrazumeva se da ceo dijagram rotira sinhronom brzinom u pozitivnom (protivkazaljkinom) smeru, a uglovi između fazora čitaju se kao fazni pomaci. **Referentni pravac je q-osa:** na njoj leži podebljani fazor $\mathbf{E}_{0f}$ — indukovana EMS praznog hoda ($1498{,}4\ \mathrm{V}$), koja po pravilu iz mini-lekcije 1 uvek pada na q-osu. Iz koordinatnog početka polaze još dva međusobno kolinearna fazora, otklonjena za ugao $\delta = 21{,}26°$ ulevo od q-ose: kraći, označen $\mathbf{I}_f$, je struja statora ($369\ \mathrm{A}$; strujni i naponski fazori su na istom crtežu u različitim razmerama), a duži, koji se pruža do tačke u kojoj se završava lanac padova napona, je fazni napon $\mathbf{U}_f$ ($1327{,}9\ \mathrm{V}$; na ovom otisku njegova oznaka nije ispisana). Pokazivač sa natpisom $\varphi = 0$ upire baš u ugao između $\mathbf{U}_f$ i $\mathbf{I}_f$ — taj ugao je nula, jer je faktor snage jediničan, pa su napon i struja u fazi (kolinearni). Mali luk uz oznaku $\delta$ pokazuje kako se čita ugao opterećenja: između q-ose (pravca $\mathbf{E}_{0f}$) i pravca $\mathbf{U}_f$/$\mathbf{I}_f$. Isprekidane linije su pravougaonik razlaganja struje na komponente: $\mathbf{I}_q = I_f\cos\delta \approx 344\ \mathrm{A}$ duž q-ose (naviše) i $\mathbf{I}_d = I_f\sin\delta \approx 134\ \mathrm{A}$ duž d-ose — na slici usmeren **ulevo**, tj. u negativnom smeru d-ose. Naponska jednačina čita se kao zatvoreni lanac fazora: od vrha $\mathbf{E}_{0f}$ ide $jX_q\mathbf{I}_q$ ($\approx 481\ \mathrm{V}$) horizontalno ulevo — normalan na $\mathbf{I}_q$, jer množenje sa $j$ zaokreće fazor za $90°$ unapred; sa njegovog vrha $jX_d\mathbf{I}_d$ ($\approx 261\ \mathrm{V}$) silazi naniže, antiparalelno sa $\mathbf{E}_{0f}$ (normalno na $\mathbf{I}_d$); tačka u kojoj se lanac završava tačno je vrh fazora $\mathbf{U}_f$ — grafička potvrda jednačine $\underline{U}_f = \underline{E}_{0f} + jX_d\underline{I}_d + jX_q\underline{I}_q$. Šta treba da zaključiš: pri $\cos\varphi = 1$ cela geometrija režima određena je sa dva broja ($U_f$ i $I_f$) — iz pravouglih trouglova ovog dijagrama direktno slede $\mathrm{tg}\,\delta = X_q I_f/U_f$ (Korak 4) i $E_{0f} = U_f\cos\delta + X_d I_f\sin\delta$ (Korak 5).
