...
Ovo je centralna lekcija zadatka. Snaga u asinhronom motoru teče kao reka koja usput gubi po malo vode na svakoj "brani". Ceo tok najlakše se prati na ekvivalentnoj šemi. Slika 36.1 prikazuje ekvivalentnu šemu asinhronog motora (po fazi) sa ucrtanim svim snagama: čitaj je sleva nadesno — ulazna snaga $P_s$ ulazi kod izvora; na rednom otporu statora $R_s$ "otpada" $P_{\mathrm{Cus}}$; na poprečnoj grani ($R_{\mathrm{Fe}}$ paralelno sa reaktansom magnećenja $X_\mu$) otpada $P_{\mathrm{Fe}}$; ono što preostane, $P_\delta$, prelazi isprekidanu liniju (vazdušni zazor) ka rotorskoj strani; na svedenom rotorskom otporu $R'_r$ otpada $P_{\mathrm{Cur}}$; na fiktivnom promenljivom otporu $\frac{1-s}{s}\cdot R'_r$ razvija se mehanička snaga konverzije $P_c$, od koje se, po odbitku mehaničkih gubitaka, dobija korisna snaga $P_m = P_c - P_{\mathrm{trv}}$.
...
Ceo bilans se najzornije vidi na tzv. Sankijevom dijagramu — "traci" čija širina predstavlja snagu, a bočne grane gubitke. Slika 36.2 prikazuje bilans za motorski režim: široka traka $P_s$ ulazi sleva; nadole se odvajaju $P_{\mathrm{Cus}}$ i $P_{\mathrm{Fe}}$ (statorski gubici); ostatak $P_\delta$ prelazi na rotorsku stranu; naviše se odvajaju $P_{\mathrm{Cur}}$ pa $P_{\mathrm{trv}}$; ono što izađe desno je korisna snaga $P_m$.
...
Ista mašina može da radi i obrnuto. Slika 36.3 prikazuje bilanse za druga dva režima — čitaj smerove strelica: **levo je generatorski režim** — mehanička snaga $P_m$ ulazi sa vratila (desno), od nje se odbijaju $P_{\mathrm{trv}}$ i $P_{\mathrm{Cur}}$ (naviše), snaga $P_\delta$ prelazi zazor ka statoru, odbijaju se $P_{\mathrm{Fe}}$ i $P_{\mathrm{Cus}}$ (nadole), a velika strelica $P_s$ izlazi **ulevo, u mrežu**; **desno je kočioni režim** — snaga ulazi u mašinu **sa obe strane** (i električna $P_s$ iz mreže i mehanička $P_m$ sa vratila) i **sva** se pretvara u gubitke unutar mašine (najviše u bakru rotora, jer je tada $s > 1$) — zato se mašina u kočenju najviše greje.
