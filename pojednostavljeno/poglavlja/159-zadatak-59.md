...
Slika 59.1 prikazuje dve šeme priključenja jednosmernog izvora na statorski namotaj spregnut u zvezdu — levo spoj a, desno spoj b.
...
> **Kako čitati sliku 59.1:** Dve šeme veze (nema osa — čitaju se elementi i put struje). Na obe šeme: tri sivo nacrtana kalema su tri fazna namotaja statora — $U$ (gore), $V$ (dole desno) i $W$ (dole levo) — spojena u zajedničku zvezdanu tačku (puna tačka u sredini); crne linije su spoljašnji provodnici, a crvene strelice i oznake prikazuju jednosmerne struje i napon. Crvena dvostrana strelica $U_{DC}$ pri vrhu označava napon priključenog jednosmernog izvora (u voltima), a znak „+" uz stezaljku $U$ kaže da je plus izvora na fazi $U$. **Leva šema (spoj a):** struja $I_{DC}$ (crvena strelica nadole uz gornji kalem; po računu $31{,}35\ \mathrm{A}$) ulazi kroz namotaj $U$, stiže do zvezdišta i tu se deli na dve jednake polovine $I_{DC}/2$ (dve crvene strelice usmerene od zvezdišta ka spolja) koje izlaze kroz namotaje $V$ i $W$; stezaljke $V$ i $W$ su pri dnu kratko spojene i zajedničkim desnim provodnikom vraćaju struju na minus izvora. Izvor, dakle, „vidi" otpornost $R_s + (R_s \parallel R_s) = \tfrac{3}{2}R_s = 0{,}341\ \Omega$. **Desna šema (spoj b):** ista struja $I_{DC}$ (po računu $27{,}15\ \mathrm{A}$) teče kroz namotaj $U$ do zvezdišta i **celom jačinom** nastavlja kroz namotaj $V$ (crvena strelica uz donji desni kalem) do stezaljke $V$ i desnim provodnikom nazad u izvor; namotaj $W$ je nepriključen — njegova stezaljka visi u vazduhu i kroz njega struje nema. Izvor vidi $2R_s = 0{,}454\ \Omega$. Šta treba da zaključiš: ista mašina, dva različita prevezivanja — u spoju a struju nose sve tri faze (dve sa po pola), u spoju b dve faze punom strujom — pa se razlikuju i geometrije magnetopobudnih sila (slike 59.2 i 59.3) i potrebni naponi izvora ($10{,}7$ prema $12{,}3\ \mathrm{V}$).
...
Sledeća slika prikazuje vektorski dijagram MPS za ovaj spoj, uz ponovljenu šemu veze.
...
> **Kako čitati sliku 59.2:** Slika ima dva dela. **Levo** je ponovljena šema spoja a (ista kao na slici 59.1 levo — struja $I_{DC}$ kroz fazu $U$, po $I_{DC}/2$ kroz faze $V$ i $W$), da se vidi odakle dolaze struje čije MPS sabiramo. **Desno** je vektorski dijagram magnetopobudnih sila. Pazi: ovo **nisu** fazori u vremenu, nego vektori u **prostoru** mašine — svaki leži u pravcu ose svoje faze, a dužina mu je srazmerna trenutnoj (ovde jednosmernoj, dakle stalnoj) struji te faze; jedinica im je amper-navojak, pa se na dijagramu porede samo relativne dužine. Dugačak crveni vektor nacrtan vertikalno je $\theta_{DC}$ — MPS faze $U$, u osi faze $U$, od pune struje $I_{DC}$. Na njegov vrh nadovezana su (sabiranje „nadovezivanjem", vektor na vektor) dva upola kraća vektora $\tfrac{\theta_{DC}}{2}$ — MPS faza $V$ i $W$: pošto su im struje negativne ($-I_{DC}/2$), okrenuti su **suprotno** od svojih osa, pa svaki zaklapa ugao od $60°$ sa pravcem ose $U$ (jedan skreće ulevo, drugi udesno, pa im se poprečne komponente poništavaju). Debela dvostruka strelica duž ose $U$, od početka $\theta_{DC}$ do vrha poslednjeg sabirka, jeste rezultanta: $\theta_{DC} + 2\cdot\tfrac{\theta_{DC}}{2}\cos 60° = \tfrac{3}{2}\theta_{DC}$ — formula (59.1). Desno na slici ispisan je i uslov zadatka: $\tfrac{3}{2}\theta_{DC} = \tfrac{3}{2}\theta_{1\max}$, tj. rezultanta jednosmernih MPS mora biti jednaka amplitudi MPS obrtnog polja praznog hoda; odatle $\theta_{DC} = \theta_{1\max}$, što daje $I_{DC} = \sqrt{2}\,I_0 = 31{,}35\ \mathrm{A}$ (korak 4). Šta treba da zaključiš: spoj a pravi tačno „zamrznuti snimak" obrtnog polja u trenutku maksimuma struje faze $U$ — ista geometrija i isti faktor $\tfrac{3}{2}$ kao kod obrtnog polja, samo što polje sada miruje.
...
> **Kako čitati sliku 59.3:** I ova slika ima dva dela. **Levo** je ponovljena šema spoja b (kao na slici 59.1 desno — struja $I_{DC}$ redno kroz faze $U$ i $V$, faza $W$ visi neiskorišćena). **Desno** je vektorski dijagram MPS — opet prostorni vektori (svaki u pravcu ose svoje faze, dužine srazmerne struji), ne vremenski fazori. Prvi crveni vektor $\theta_{DC}$ je MPS faze $U$, nacrtan duž ose faze $U$ (vertikalno). Na njegov vrh nadovezan je drugi vektor iste dužine $\theta_{DC}$ — MPS faze $V$: iako ose faza $U$ i $V$ zaklapaju $120°$, struja kroz $V$ teče **suprotno** od referentnog smera ($i_V = -I_{DC}$), pa je njen vektor okrenut za $180°$ i sa prvim zaklapa $180° - 120° = 60°$ — zato je na slici drugi vektor otklonjen za $60°$, a ne za $120°$ (ovo je najčešća zamka ovog zadatka). Dugačka kosa strelica od početka prvog do vrha drugog vektora je rezultanta; po pravilu sabiranja vektora (korak 7) njena dužina je $\sqrt{3}\,\theta_{DC} \approx 1{,}73\,\theta_{DC}$ i leži na simetrali ugla između sabiraka. Uz nju je ispisan uslov zadatka: $\sqrt{3}\,\theta_{DC} = \tfrac{3}{2}\theta_{1\max}$, iz koga sledi $I_{DC} = \sqrt{3/2}\,I_0 = 27{,}15\ \mathrm{A}$ (korak 8). Šta treba da zaključiš: dve faze pod $60°$ „iskorišćavaju" struju bolje nego spoj a (faktor $\sqrt{3} = 1{,}73$ prema $\tfrac{3}{2} = 1{,}5$), pa je za isto polje potrebna manja struja izvora — ali kroz veću otpornost $2R_s$, te napon izvora ispada nešto veći ($12{,}3$ prema $10{,}7\ \mathrm{V}$).
