...
Slika ispod prikazuje momentne karakteristike $M(n_2)$ — moment u funkciji brzine rotora $n_2$ — izračunate po tačnom izrazu (37.1) i po uprošćenom izrazu (37.5).

![Momentne karakteristike asinhronog motora po potpunoj i uprošćenoj ekvivalentnoj šemi](../slike/fig-37.1.png)

**Slika 37.1 —** Karakteristike momenta asinhronog motora: crvena kriva („Potpuna ekvivalentna šema") — po tačnom izrazu (37.1); siva kriva („Uprošćena ekvivalentna šema") — uz zanemarenje struje magnetisanja $I_{\mu}$, izraz (37.5).

> **Kako čitati sliku 37.1:** Na horizontalnoj osi je brzina obrtanja rotora $n_2$ u $\mathrm{min^{-1}}$, od $0$ (polazak, $s=1$, levi kraj) do sinhrone brzine $1500\ \mathrm{min^{-1}}$ ($s=0$, desni kraj); na vertikalnoj osi je moment $M$ u $\mathrm{Nm}$, skala do $900\ \mathrm{Nm}$. Donja, crvena kriva (natpis na slici: „Potpuna ekvivalentna šema") je tačan izraz (37.1): počinje od polaznog momenta $M_{\mathrm{p}} = 342\ \mathrm{Nm}$ pri $n_2 = 0$, penje se do prevalnog momenta $M_{\mathrm{pr}} = 827{,}5\ \mathrm{Nm}$ na $n_2 = (1 - 0{,}189) \cdot 1500 \approx 1217\ \mathrm{min^{-1}}$, pa strmo pada na nulu u sinhronizmu. Gornja, siva kriva (natpis: „Uprošćena ekvivalentna šema") je izraz (37.5) bez struje magnetisanja: počinje od $M'_{\mathrm{p}} = 354\ \mathrm{Nm}$, vrh joj je $M'_{\mathrm{pr}} = 869\ \mathrm{Nm}$ na praktično istoj brzini ($s'_{\mathrm{pr}} = 0{,}185$); svuda ide malo **iznad** crvene (momenti precenjeni za 3–7 %), ali su oblik krive i položaj maksimuma praktično isti. Zelena tačkasta vertikala blizu desnog kraja, sa dve suprotstavljene strelice i oznakom $s_{\mathrm{n}}$, markira uzan pojas nominalnog klizanja: nominalna radna tačka leži na strmoj desnoj grani, pri $n_2 = (1 - 0{,}029) \cdot 1500 \approx 1457\ \mathrm{min^{-1}}$. Šta treba da zaključiš: zanemarenje struje magnetisanja samo blago „podigne" celu krivu, ne menjajući ni njen oblik ni položaj prevala — zato je uprošćena šema (37.5) upotrebljiva za sva klizanja, a razlika je najmanja baš u radnoj oblasti uz sinhronu brzinu.
