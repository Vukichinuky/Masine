...
Sve rečeno najlakše se vidi na slici koja sledi: na njoj su nacrtane obe pune momentne karakteristike, njihove linearne aproksimacije i prava konstantnog momenta tereta sa radnim tačkama **1** i **2**.
...
> **Kako čitati sliku 46.1:** Na horizontalnoj osi je klizanje $s$ (bezdimenziono), od $0$ (sinhronizam, levo) do $1$ (zakočen rotor, desno) — pazi, brzina obrtanja duž ove ose **opada** udesno; na vertikalnoj osi je relativni moment $M/M_n$ (takođe bezdimenzion, skala do $4$). Ljubičasta kriva (oznaka $R_{\mathrm{r}}$) je puna momentna karakteristika sa sopstvenim rotorskim otporom: vrh $M_{pr} \approx 2\,M_n$ dostiže na prevalnom klizanju $s_{pr} \approx 0{,}2$, pa dalje opada. Svetloplava kriva (oznaka $R_{\mathrm{r}} + R_{rd}$) je karakteristika sa dodatnim otporom: ima **isti vrh** od $\approx 2\,M_n$ (prevalni moment ne zavisi od rotorskog otpora), ali ga dostiže tek oko $s \approx 1$ — kriva je „razvučena" po osi klizanja. Crvena prava $2 M_{pr}\, s/s_{pr}$ i zelena prava $2 M_{pr}\, s/s'_{pr}$ su linearne aproksimacije radnih delova tih dveju krivih: obe prolaze kroz koordinatni početak, crvena strmo (mali otpor — strma karakteristika), zelena znatno položenije (uvećani otpor). Siva horizontala $M_t = \mathrm{konst.}$ leži na visini $M/M_n = 1$ (teret jednak nazivnom momentu). Crni kružići su stacionarne radne tačke: tačka **1** na preseku sa crvenom pravom, pri klizanju $s = 0{,}05$ (crvena strelica nadole do oznake $s$ na osi), i tačka **2** na preseku sa zelenom pravom, pri klizanju $s' = 0{,}25$ (zelena strelica do oznake $s'$) — tačno vrednosti iz Koraka 5. Šta treba da zaključiš: pri konstantnom momentu tereta petostruko veći rotorski otpor daje tačno petostruko veće klizanje — radna tačka sklizne po horizontali tereta iz 1 u 2, motor uspori sa $950$ na $750\ \mathrm{min^{-1}}$, a visina prevala ostaje ista.
...
Sledeća slika prikazuje to kolo u oba režima, jedno pored drugog — pre i posle dodavanja otpora.
...
> **Kako čitati sliku 46.2:** Levo je ekvivalentno kolo jedne faze rotora **pre** regulacije, desno **posle**, a žuto uokvireni natpis na vrhu, $M = M' = M_t = \mathrm{konst.}$, podseća pod kojim uslovom poređenje važi. U oba kola struktura je ista, pa je pratimo od izvora: crvena vertikalna strelica sa oznakom $+$ je indukovana elektromotorna sila $\overline{E}_{\mathrm{rfk}}$ (u voltima; EMS pri zakočenom rotoru — ona ne zavisi od klizanja, zato je u oba kola identična), za njom redno dolazi rasipna reaktansa rotora $X_{\gamma r}$ (kalem, u omima), pa promenljivi **fiktivni otpor** sa kosom strelicom (simbol promenljivosti sa klizanjem): levo $R_{\mathrm{r}}/s$, desno $(R_{\mathrm{r}} + R_{rd})/s'$, za koji na samoj slici piše da je $= R_{\mathrm{r}}/s$. Gornja crvena strelica označava rotorsku struju: levo $\overline{I}_{\mathrm{rf}}$, desno $\overline{I}{}'_{\mathrm{rf}} = \overline{I}_{\mathrm{rf}}$. Brojčano su fiktivni otpori zaista jednaki: $R_{\mathrm{r}}/s = 0{,}2/0{,}05 = 4\ \mathrm{\Omega}$ i $(R_{\mathrm{r}}+R_{rd})/s' = 1{,}0/0{,}25 = 4\ \mathrm{\Omega}$ (Korak 7) — desno kolo je element po element identično levom, pa su jednake i struje. Plave strelice sleva označavaju aktivnu snagu koja ulazi u kolo: $P_\delta$ levo i $P'_\delta = P_\delta$ desno (u oba režima $120\ \mathrm{kW}$). Šta treba da zaključiš: pri konstantnom momentu tereta dodavanje rotorskog otpora ne menja ni struju rotora ni snagu koju rotor prima kroz zazor — menja se samo raspodela te iste snage unutar rotorskog kola na toplotu i mehaniku, srazmerno klizanju.
