...
Sledeća slika prikazuje taj mehanizam grafički: dve momentne karakteristike motora (pri nazivnom i pri sniženom naponu) i kvadratnu krivu tereta, sa radnim tačkama M i N.
...
> **Kako čitati sliku 38.1:** Na vodoravnoj osi je brzina obrtanja u $\mathrm{rad/s}$ (od 0 do 200), na uspravnoj moment u $\mathrm{Nm}$ (od 0 do 90); sve krive su crne i razlikuju se po natpisima. **Gornja kriva** (natpis "za $U_{sn}$") je momentna karakteristika motora pri nazivnom naponu $230\ \mathrm{V}$: kreće od $\approx 52\ \mathrm{Nm}$ pri $\omega = 0$ (polazni moment), raste do prevalnog momenta $\approx 85\ \mathrm{Nm}$ na $\approx 129\ \mathrm{rad/s}$ (kroz vrh je povučena isprekidana vertikala; to je prevalna brzina $\omega_s(1 - s_{\mathrm{pr}})$ sa $s_{\mathrm{pr}} \approx 0{,}314$ — izračunato u Proveri smisla), pa strmo pada na nulu na sinhronoj brzini $\omega_s = 188{,}5\ \mathrm{rad/s}$. **Donja, "spljoštena" kriva** (natpis "za $U_{s1} < U_{sn}$") je ista karakteristika pri sniženom naponu $\approx 89{,}4\ \mathrm{V}$: pošto je $M \propto U^2$, sve njene ordinate su $(89{,}4/230)^2 \approx 0{,}15$ puta manje (vrh $\approx 13\ \mathrm{Nm}$), ali **na istim brzinama** — nula je i dalje na $188{,}5\ \mathrm{rad/s}$, vrh i dalje na $\approx 129\ \mathrm{rad/s}$, jer klizanje u formuli (38.1) ne stoji uz napon. **Rastuća kriva** (natpis "teret", strelica pokazuje na nju) je kvadratna karakteristika radne mašine $M_{\mathrm{opt}} = c\,\omega^2$. Radne tačke su preseci krive tereta sa krivom motora, obeleženi punim kružićima: **M** $= (183\ \mathrm{rad/s};\ 17{,}7\ \mathrm{Nm})$ na nazivnoj karakteristici (koraci 2 i 5) i **N** $= (150\ \mathrm{rad/s};\ 11{,}9\ \mathrm{Nm})$ na sniženoj (koraci 6 i 7). Strelice uz tačku M opisuju prelaz: kratka strelica **nadole** — u trenutku snižavanja napona moment motora "propadne" na spljoštenu krivu dok je brzina još $183\ \mathrm{rad/s}$ (motor tada daje manje nego što teret traži) — a strelica **duž krive tereta ulevo** pokazuje kako se radna tačka zatim spušta ka N, gde se momenti ponovo izjednače. Šta treba da zaključiš: promena napona ne pomera karakteristiku po brzini nego je samo "spljošti" kvadratom napona; nova ravnoteža sa kvadratnim teretom nastaje na nižoj brzini, a tačka N ($150\ \mathrm{rad/s}$) leži desno od prevalne brzine ($\approx 129\ \mathrm{rad/s}$), dakle na stabilnoj grani — regulacija je izvodljiva, ali uz veliko klizanje od $20{,}4\ \%$.
