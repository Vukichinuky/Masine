...
Sledeća slika prikazuje vektorski dijagram napona nadpobuđenog sinhronog motora, konstruisan tačno po receptu iz Mini-lekcije 6. Čitaj je ovako: iz zajedničke početne tačke (dole) polaze fazor napona mreže $\bar{U}_f$ (vertikalno naviše), fazor EMS $\bar{E}_{0f}$ (duži od $U_f$, zakrenut za ugao opterećenja $\delta$ iza napona — udesno, jer je mašina motor) i fazor struje $\bar{I}_f$ (zakrenut za ugao $\varphi$ **ispred** napona — ulevo, jer je motor nadpobuđen). Fazor pada napona $jX_s\bar{I}_f$ spaja vrh $\bar{E}_{0f}$ sa vrhom $\bar{U}_f$ i zatvara naponsku jednačinu $\bar{U}_f = \bar{E}_{0f} + jX_s\bar{I}_f$. Isprekidana linija je pravac struje: oznaka pravog ugla na njoj podseća da je $jX_s\bar{I}_f$ uvek upravan na $\bar{I}_f$ (množenje sa $j$ = zakretanje za $90^\circ$).
...
Sa slike se jasno uočava: kada je $E_{0f} > U_f$ (nadpobuđen režim), struja $\bar{I}_f$ prednjači naponu, tj. faktor snage je kapacitivan i motor proizvodi reaktivnu snagu. Takođe uočavamo **trougao napona** sa temenima u početnoj tački, vrhu $\bar{U}_f$ i vrhu $\bar{E}_{0f}$: njegove stranice su $U_f$, $E_{0f}$ i $X_s I_f$, a ugao između stranica $U_f$ i $E_{0f}$ je upravo $\delta$.
