...
Sledeća slika prikazuje familiju statičkih karakteristika momenta $M(n)$ jednog te istog motora za četiri učestanosti (5, 20, 35 i 50 Hz) pri $U/f = \mathrm{konst}$. Čitajte je ovako: na horizontalnoj osi je brzina obrtanja $n$ u $\mathrm{min^{-1}}$, na vertikalnoj moment $M$ u $\mathrm{Nm}$; svaka obojena kriva je karakteristika za jednu učestanost — smanjenjem učestanosti cela kriva se paralelno pomera ulevo (ka manjim brzinama), ali joj vrh (prevalni moment, ovde oko $80\ \mathrm{Nm}$, što je upravo naših $79{,}5\ \mathrm{Nm}$) ostaje na istoj visini. Horizontalna siva linija je konstantan moment tereta $M_{\mathrm{T}}$ (naša dizalica, oko $21\ \mathrm{Nm}$). Radna tačka pogona je presek linije tereta i stabilne (strmo opadajuće) grane krive: tačka označena sa „1" na krivoj za 35 Hz, pri brzini $n_{\mathrm{T}}^{(35)}$ tik iznad $1000\ \mathrm{min^{-1}}$ — upravo brzina koju u ovom zadatku računamo.
