...
Sledeća slika prikazuje sva tri stadijuma ovog postupka, sleva nadesno — pratite kako isti trougao menja "ulogu": napon → struja → snaga; detaljno čitanje dato je odmah ispod slike.
...
> **Kako čitati sliku 22.1:** Slika sadrži tri dijagrama — isti pravougli trougao u tri "uloge". **Levi (naponski) dijagram:** sve stranice su naponi (u $\mathrm{V}$, odnosno u r.j.); vertikalni fazor je napon mreže $U$ (referenca), kratki fazor pod uglom $\varphi$ desno od njega je struja $I$ (ucrtana radi ugla), iz vrha $U$ nadovezan je pad napona $IX_{\mathrm{s}}$ (normalan na struju), a zbir je elektromotorna sila $E_0$ — najduži fazor, gore levo; ugao $\delta$ između $E_0$ i $U$ je ugao opterećenja. **Srednji (strujni) dijagram:** sve stranice levog trougla podeljene su sa $X_{\mathrm{s}}$ i dijagram je zarotiran za $90^{\circ}$, pa stranice sada imaju dimenziju struje (u $\mathrm{A}$): horizontalna stranica uz osu je $U/X_{\mathrm{s}}$, kosa je $E_0/X_{\mathrm{s}}$ (pod uglom $\delta$ prema horizontali), a treća stranica je sama struja $I$; njene projekcije su $I\cos\varphi$ (vertikalna strelica — aktivna komponenta) i $I\sin\varphi$ (duž horizontalne ose — reaktivna komponenta). **Desni dijagram (snage):** množenjem naponom $U$ stranice postaju snage — crvene ose su $P = UI\cos\varphi$ (vertikalna) i $Q = UI\sin\varphi$ (horizontalna), obe u relativnim jedinicama (oznaka [pu] = r.j.); fazor iz koordinatnog početka $0$, pod uglom $\varphi$ prema P-osi, je prividna snaga $S = UI$, a kosi fazor dužine $E_0U/X_{\mathrm{s}}$ kreće iz tačke $-U^2/X_{\mathrm{s}}$ na negativnom delu Q-ose pod uglom $\delta$. Crvena polukružnica poluprečnika $1$ (od $-1$ do $1$ na Q-osi) je geometrijsko mesto radnih tačaka sa nazivnom strujom statora, $s = u \cdot i = 1$. U brojevima ovog zadatka: pomeraj centra je $u^2/x_{\mathrm{s}} = 0{,}714$ r.j., a dužina kosog fazora pri nazivnoj pobudi $e_0 u/x_{\mathrm{s}} = 1{,}54$ r.j. **Šta treba da zaključiš:** vrh fazora $S$ jeste radna tačka $(q,\ p)$ pogonske karte — zato je granica struje statora kružnica oko koordinatnog početka, a granice konstantne pobude su kružnice oko tačke $(-u^2/x_{\mathrm{s}},\ 0)$ poluprečnika $e_0 u/x_{\mathrm{s}}$.
...
Pogonska karta je presek svih ovih uslova. Sledeća slika prikazuje kako to izgleda u opštem slučaju: šrafirana oblast je dozvoljeno područje rada, brojevi $1$–$6$ na konturi odgovaraju upravo gornjoj listi ograničenja, a tačka $N$ je **nazivna radna tačka**; detaljno čitanje dato je odmah ispod slike.
...
> **Kako čitati sliku 22.2:** Ose: horizontalna je reaktivna snaga $Q$, vertikalna aktivna snaga $P$, obe u relativnim jedinicama (oznaka "pu" na osama); desna poluravan ($Q > 0$) je nadpobuđeni (induktivni), leva potpobuđeni (kapacitivni) režim. Tanka zelena polukružnica poluprečnika $1$ oko koordinatnog početka je granica maksimalne struje statora (broj **1**), a mala isprekidana zelena kružnica dole levo je kružnica minimalne pobude iz iste familije kružnica pobude. Kontura šrafirane (dozvoljene) oblasti, obilazak od gore desno: **4** — horizontala maksimalne snage turbine (gornja ivica, završava se u tački $N$); **2** — luk maksimalne struje pobude (GMP) od $N$ naniže (na ovoj skici on je unutar kružnice 1, pa statorska granica dodiruje oblast samo u tački $N$); **5** — horizontala minimalne snage turbine (donja ivica); **3** — kratki luk minimalne struje pobude dole levo; **6** — kriva statičke stabilnosti (leva ivica). Tačka $N$ (kružić) je nazivna radna tačka: u njoj su i aktivna i reaktivna snaga jednake nazivnim vrednostima, a isprekidana zelena poluprava iz koordinatnog početka kroz $N$ zaklapa sa P-osom nazivni fazni ugao $\varphi_{\mathrm{n}}$ (za naš generator $\cos\varphi_{\mathrm{n}} = 0{,}8$, tj. $\varphi_{\mathrm{n}} \approx 36{,}9^{\circ}$, pa je $N = (0{,}6;\ 0{,}8)$). **Šta treba da zaključiš:** svaki deo konture je "vlasništvo" tačno jednog od šest ograničenja — kad radna tačka pređe konturu, sa slike se odmah vidi koje ograničenje bi bilo prekršeno.
...
Sledeća slika prikazuje gotovu pogonsku kartu upravo ovog turbogeneratora, sa svim pomoćnim lukovima konstrukcije; detaljno čitanje dato je odmah ispod slike.
...
> **Kako čitati sliku 22.3:** Ose: horizontalna je $q$ u r.j. — leva strelica nosi oznaku $q_{\mathrm{cap}}$ (kapacitivni/potpobuđeni režim), desna $q_{\mathrm{ind}}$ (induktivni/nadpobuđeni), sa podeokom $1$ desno; vertikalna je $p$ u r.j., sa podeocima $0{,}1$–$1$. Elementi slike: tanka kružnica kroz $1$ na obe ose je granica struje statora ($p^2 + q^2 = 1$); horizontala pMAX na visini $p = 0{,}8$ i pMIN na $p = 0{,}24$ su granice turbine; tačka $q_{\mathrm{c}} = -0{,}714$ na Q-osi (dole levo) je zajednički centar svih kružnica pobude. Iz $q_{\mathrm{c}}$ su povučene strelice poluprečnika: $r_{\mathrm{MAX}} = 0{,}9$ do tačke $E$, kraća $r_{\mathrm{MIN}} = 0{,}34$ do tačke $D$, dugačka $r_{\mathrm{GMP}} = 1{,}54$ do luka GMP (dole desno; taj luk odozgo prolazi kroz nazivnu tačku $N$ i podebljan se spušta do prave pMIN) i sasvim mala $r_{\mathrm{GmP}} = 0{,}154$ u maloj polukružnici GmP (minimalna pobuda) — koja cela ostaje ispod prave pMIN i zato ne učestvuje u konturi; kratka horizontala pri dnu, na visini $p \approx 0{,}05$, označava granicu stabilnosti pri minimalnoj pobudi ($p_{\mathrm{PGS\,min}} = 0{,}054$ r.j. iz Koraka 6 — takođe neaktivnu). Tanki lukovi označeni $0{,}4$; $0{,}5$; …; $0{,}9$ gore levo su pomoćne kružnice pobude: presek luka poluprečnika $r$ sa horizontalom $p = r - 0{,}1$ (kratke horizontalne linije na $p = 0{,}3$–$0{,}7$) daje po jednu tačku granice stabilnosti, po pravilu $r = p + 0{,}1$ iz Koraka 6; podebljana kriva kroz te tačke, od $E = (-0{,}302;\ 0{,}8)$ do $D = (-0{,}473;\ 0{,}24)$, jeste kriva PGS. Isprekidana vertikala kroz $N$ obeležava $q_{\mathrm{n}} = 0{,}6$. Kontura dozvoljene oblasti: pMAX od $E$ do $N$ → luk GMP od $N$ naniže → pMIN do $D$ → kriva PGS od $D$ nazad do $E$. **Šta treba da zaključiš:** unutar te konture generator sme trajno da radi; jedinična kružnica učestvuje samo u tački $N$ (rotorska granica je ovde "stroža" od statorske), a granica minimalne pobude uopšte nije aktivna.
