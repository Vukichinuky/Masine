...
Slika koja sledi prikazuje kompletan vektorski dijagram takvog generatora: zelenim su nacrtani električni fazori (napon, EMS, padovi napona), crvenim magnetopobudne sile, a plavim struja statora. Čitaj je ovako: kreni od horizontalnog fazora $\overline{U}_f$ (napon mreže, referentni pravac). Ispod njega, pod uglom $\varphi$, leži struja $\overline{I}_{sf}$ (kasni — induktivno opterećenje). Na vrh $\overline{U}_f$ nadovezan je pad $\overline{I}_{sf}\cdot R_s$ (paralelan struji), pa na njega pad $j\cdot\overline{I}_{sf}\cdot X_{\gamma s}$ (normalan na struju — vidi oznaku $\pi/2$ dole desno); vrh te "stepenice" je vrh fazora $\overline{E}_f$, koji prednjači naponu za ugao $\gamma$. U crvenom delu: $\overline{F}_s$ leži na pravcu struje (u fazi s njom), $\overline{F}_{rez}$ prednjači fazoru $\overline{E}_f$ za $\pi/2$ (oznaka gore), a $\overline{F}_r$ se dobija kao $\overline{F}_{rez} + (-\overline{F}_s)$ — na slici je $-\overline{F}_s$ nacrtan kao produžetak pravca struje na suprotnu stranu, i tačkastim linijama je naznačen paralelogram sabiranja.
