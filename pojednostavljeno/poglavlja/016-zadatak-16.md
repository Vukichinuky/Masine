...
Slika 16.1 prikazuje vektorski dijagram prvog generatora, iz koga ćemo prečicom pročitati traženi proizvod.
...
> **Kako čitati sliku 16.1:** Boje razdvajaju veličine: **plavo** je struja, **zeleno** su naponi; dužina svake strelice je efektivna vrednost (struja u $\mathrm{A}$ u svojoj razmeri, naponi u $\mathrm{V}$), a fazori se obrću suprotno kazaljci na satu. Referentni pravac je horizontala — pravac napona. Plavi fazor struje $\overline{I}_{sf}' = I\cos\varphi = 80\ \mathrm{A}$ i zeleni fazor napona $\overline{U}_{sf}' = \overline{U}_{sf} = 220\ \mathrm{V}$ leže na **istom pravcu i u istom smeru** (na slici su nacrtani jedan u produžetku drugog samo da se strelice ne bi preklapale — kao fazori, oba polaze iz koordinatnog početka): struja i napon su u fazi, jer prvi generator daje samo aktivnu snagu. Iz vrha napona diže se vertikalni zeleni fazor pada $j\cdot\overline{I}_{sf}'\cdot X_s$: množenje sa $j$ zakreće ga za $+90^\circ$ u odnosu na struju, a dužina mu je $I_{sf}'X_s = 80 \cdot 0{,}8 = 64\ \mathrm{V}$. Kosi zeleni fazor od koordinatnog početka do vrha vertikale je elektromotorna sila $\overline{E}_{0sf}'$ — hipotenuza pravouglog trougla sa katetama $220\ \mathrm{V}$ i $64\ \mathrm{V}$; luk sa strelicama kod početka kotira ugao opterećenja $\delta'$ između $\overline{E}_{0sf}'$ i napona (usput, mada nam pojedinačno neće trebati: $E_{0sf}' = \sqrt{220^2 + 64^2} \approx 229\ \mathrm{V}$, $\delta' = \mathrm{arctg}(64/220) \approx 16{,}2^\circ$). **Šta treba da zaključiš:** projekcija hipotenuze $\overline{E}_{0sf}'$ na horizontalu jednaka je kateti-naponu, pa je $E_{0sf}'\cos\delta' = U_{sf} = 220\ \mathrm{V}$ — tačno proizvod koji formuli za sinhronizirajući moment treba, bez računanja $E_{0sf}'$ i $\delta'$ ponaosob.
...
Slika 16.2 prikazuje vektorski dijagram drugog generatora — iz njega ćemo pročitati obe potrebne činjenice.
...
> **Kako čitati sliku 16.2:** Konvencije su iste kao na slici 16.1 (plavo — struja, zeleno — naponi; horizontala = pravac napona; rotacija fazora suprotno kazaljci, pa fazor zakrenut naniže **kasni**). Plavi fazor struje $\overline{I}_{sf}'' = I\sin\varphi = 60\ \mathrm{A}$ pokazuje pravo **naniže**: struja kasni za naponom tačno $90^\circ$ — čisto reaktivna (induktivna) struja. Zeleni fazori leže svi na horizontali: prva strelica je napon $\overline{U}_{sf}'' = \overline{U}_{sf} = 220\ \mathrm{V}$; na nju se nadovezuje pad $j\cdot\overline{I}_{sf}''\cdot X_s$ dužine $60 \cdot 0{,}8 = 48\ \mathrm{V}$ — zakretanjem fazora koji gleda naniže za $+90^\circ$ dobija se fazor koji gleda **udesno**, dakle kolinearan i istosmeran sa naponom; iznad njih je, kao paralelna strelica pune dužine, elektromotorna sila $\overline{E}_{0sf}'' = 220 + 48 = 268\ \mathrm{V}$. Male naspramne strelice sa oznakom $\delta'' = 0$ pokazuju da između $\overline{E}_{0sf}''$ i $\overline{U}_{sf}$ **nema ugla** — mali razmak na crtežu postoji samo da bi se kolinearni fazori uopšte videli. **Šta treba da zaključiš:** čisto reaktivno opterećenje ne pravi ugao opterećenja ($\delta'' = 0$, pa je $\cos\delta'' = 1$), a vektorska jednačina degeneriše u obično sabiranje brojeva, $E_{0sf}'' = 268\ \mathrm{V} > U_{sf}$ — generator je natpobuđen i upravo zato ispada "najkrući".
