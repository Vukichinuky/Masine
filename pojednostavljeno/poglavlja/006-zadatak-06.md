...
Sledeća slika prikazuje vektorski dijagram nadpobuđenog generatora (naš prvi, induktivni režim). Čitaj je ovako: zelenim su nacrtane električne veličine — napon $\overline{U}_f$ na realnoj osi, na njega se nadovezuju padovi $\overline{I}_{sf} R_s$ (paralelan struji) i $j\overline{I}_{sf}X_{\gamma s}$ (normalan na struju), a njihov zbir je EMS $\overline{E}_f$; plavim je struja $\overline{I}_{sf}$, koja **kasni** za naponom za ugao $\varphi$ (zato je ispod realne ose); crvenim su MPS: $\overline{F}_s$ leži tačno na pravcu struje, $\overline{F}_{rez}$ stoji $90^\circ$ **ispred** EMS, a $\overline{F}_r$ se dobija kao $\overline{F}_{rez} + (-\overline{F}_s)$ — tačkasti paralelogram pokazuje to sabiranje. Uoči koliko je $\overline{F}_r$ **duži** od $\overline{F}_{rez}$: rotor mora da savlada razmagnetišuću reakciju indukta.
...
Naredna slika je isti dijagram za podpobuđen generator (naš drugi, kapacitivni režim). Čitaj je isto kao prethodnu, s tim što sada struja $\overline{I}_{sf}$ (plavo) **prednjači** naponu $\overline{U}_{sf}$ za ugao $\varphi$ (iznad realne ose), pa se sa njom "gore" popela i MPS statora $\overline{F}_s$. Posledica: pri vektorskom oduzimanju $\overline{F}_r = \overline{F}_{rez} - \overline{F}_s$ rotorska MPS ispada **kraća** od rezultantne — stator sada pomaže magnećenju.
...
Sledeća slika prikazuje vektorski dijagram mašine u kratkom spoju. Čitaj je ovako: struja $\overline{I}_{sf}$ (plavo) je referentna, na realnoj osi; EMS $\overline{E}_{sf}$ (zeleno) je zbir malog pada $\overline{I}_{sf}R_s$ (uz struju) i velikog pada $j\overline{I}_{sf}X_{\gamma s}$ (normalno na struju), pa je skoro vertikalna — ugao $\varphi_K$ između EMS i struje je blizu $90^\circ$ (skoro čisto induktivno kolo); $\overline{F}_{rez}$ (crveno) je $90^\circ$ ispred EMS, dakle skoro horizontalna i **nasuprot** struji; $-\overline{F}_s$ leži tačno nasuprot struji, pa su $\overline{F}_{rez}$ i $-\overline{F}_s$ skoro kolinearni — njihov zbir $\overline{F}_r$ je skoro prosta razlika dužina. To je slika rečenice "u kratkom spoju reakcija indukta direktno razmagnetiše mašinu".
