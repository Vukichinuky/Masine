...
Sledeća slika prikazuje fazorski dijagram ovog režima — nadpobuđen generator na induktivnom potrošaču — i to „iz oba ugla": donji deo dijagrama gradi napon $\underline{U}$ **sa strane potrošača**, gornji deo gradi $\underline{E}_0$ **sa strane generatora**.
...
> **Kako čitati sliku 26.1:** Svi fazori predstavljaju fazne veličine režima a) i polaze iz zajedničke početne tačke na dnu crteža; dužina fazora odgovara efektivnoj vrednosti (naponi u $\mathrm{V}$, struja u $\mathrm{A}$, u svojoj razmeri), smer rotacije je suprotan kazaljci na satu, pa fazor zakrenut suprotno kazaljci u odnosu na drugi — **prednjači** mu. Referentni fazor je napon $\underline{U}$, nacrtan vertikalno ($U_{\mathrm{f}} = 3464\ \mathrm{V}$). Struja $\underline{I}$ ($120\ \mathrm{A}$) zakrenuta je udesno (u smeru kazaljke) od njega — **kasni** za naponom za $\varphi = 36{,}87^\circ$, jer je potrošač induktivan. Donji, potrošački deo čitaj kao lanac od početne tačke: na pravcu struje leži pad $R_{\mathrm{opt}}\underline{I}$ (u fazi sa strujom; $23{,}09 \cdot 120 \approx 2771\ \mathrm{V}$), na njega se pod pravim uglom nadovezuje $jX_{\mathrm{opt}}\underline{I}$ (prednjači struji za $90^\circ$; $17{,}32 \cdot 120 \approx 2078\ \mathrm{V}$), a njihov zbir je tačno $\underline{U}$. Gornji, generatorski deo: od vrha $\underline{U}$ nastavlja pad $jX_{\mathrm{s}}\underline{I}$ (takođe normalan na struju, ali mnogo duži: $58{,}08 \cdot 120 \approx 6970\ \mathrm{V}$) do vrha $\underline{E}_0$. Fazor $\underline{E}_0$ ($E_{0\mathrm{f}} = 9463\ \mathrm{V}$) najduži je na slici — skoro tri puta duži od $\underline{U}$, što je slika jake nadpobuđenosti — i zakrenut je suprotno kazaljci od $\underline{U}$ za ugao opterećenja $\delta = 36{,}1^\circ$: **prednjači** naponu, kako i mora kod generatora. Tačkasta linija pri vrhu je pomoćna linija za očitavanje projekcija, a ugao $\varphi$ ucrtan gore (između nje i pravca $jX_{\mathrm{s}}\underline{I}$) jednak je uglu $\varphi$ dole — uglovi sa uzajamno normalnim kracima. Šta treba da zaključiš: jedan isti crtež istovremeno prikazuje obe jednačine, $\underline{U} = (R_{\mathrm{opt}} + jX_{\mathrm{opt}})\underline{I}$ i $\underline{E}_0 = \underline{U} + jX_{\mathrm{s}}\underline{I}$, i golim okom se vidi zašto na pasivnoj mreži pobuda mora biti ogromna — najveći deo elektromotorne sile „pojede" pad na sopstvenoj sinhronoj reaktansi.
...
Sledeća slika prikazuje tu zamenu.
...
> **Kako čitati sliku 26.2:** Levo je stvarno stanje posle prevezivanja: veliki krug je simbol mašine, a u njemu su tri statorska namotaja nacrtana u **trouglu**; oznake $E_{0\mathrm{f1}}$ i $X_{\mathrm{s}}$ kazuju da je svaka stranica trougla jedan namotaj sa elektromotornom silom $E_{0\mathrm{f1}} = 9{,}46\ \mathrm{kV}$ i sinhronom reaktansom $X_{\mathrm{s}} = 58{,}08\ \mathrm{\Omega}$. Iz tri priključka na obodu (mali kružići) izlaze tri voda ka potrošaču: svaka grana potrošača je redna veza otpornika $R_{\mathrm{opt}} = 23{,}09\ \mathrm{\Omega}$ (pravougaonik) i induktivne reaktanse $X_{\mathrm{opt}} = 17{,}32\ \mathrm{\Omega}$ (kalem), a sve tri grane sastaju se u zajedničkoj tački skroz desno — potrošač je u zvezdi. Dvosmerna strelica $\Leftrightarrow$ u sredini označava ekvivalenciju: gledano sa priključaka (po linijskim naponima i strujama) leva i desna šema su potpuno neraspoznatljive. Desno je ekvivalent: ista mašina predstavljena namotajima u **zvezdi**, sa parametrima po fazi $E_{0\mathrm{f1e}} = E_{0\mathrm{f1}}/\sqrt{3} = 5463\ \mathrm{V}$ i $X_{\mathrm{s1e}} = X_{\mathrm{s}}/3 = 19{,}36\ \mathrm{\Omega}$ (Korak 9), priključena na potpuno isti potrošač. Šta treba da zaključiš: transformacija trougao → zvezda (EMS podeljena sa $\sqrt{3}$, impedansa sa $3$) pretvara nezgodno mešovito kolo u prosto redno pofazno kolo — a struja tog pofaznog kola je upravo linijska struja stvarne mašine, dok je struja svakog stvarnog namotaja u trouglu $\sqrt{3}$ puta manja.
