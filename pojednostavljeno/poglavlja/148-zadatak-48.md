...
Sledeća slika prikazuje upravo tu uprošćenu šemu.
...
> **Kako čitati sliku 48.1:** Ovo je šema **jedne faze**, pa su svi elementi fazne veličine. Pratimo struju od ulaza do izlaza: skroz levo je izvor faznog napona $\overline{U}_{sf}$ (u voltima; vertikalna crvena strelica sa oznakom $+$ pokazuje usvojeni referentni smer napona). Iz njega kroz celo kolo teče jedna jedina struja, označena gore levo kao $\overline{I}_{sf} = \overline{I}{}'_{rf}$ (u amperima, strelica daje referentni smer) — jednakost statorske i svedene rotorske struje je direktna posledica toga što je kolo redno, bez ijednog čvora grananja. Struja najpre prolazi kroz rasipnu reaktansu statora $X_{\gamma s}$ (u omima), zatim prelazi isprekidanu crvenu vertikalnu liniju — ona označava **vazdušni zazor**, granicu između statora i rotora; horizontalna strelica $P_\delta$ pokazuje da je sva aktivna snaga koja pređe tu granicu upravo snaga obrtnog polja (u vatima). Slede svedena rasipna reaktansa rotora $X'_{\gamma r}$ i, skroz desno, promenljiva otpornost $R'_r/s$ — kosa strelica preko otpornika je standardni simbol promenljivog elementa i podseća da se ta otpornost menja sa klizanjem (pri malom $s$ je ogromna, pri polasku $s=1$ jednaka je samo $R'_r$). Na reaktansama se aktivna snaga ne troši, pa se **sva** aktivna snaga kola razvija na $R'_r/s$: $P_\delta = 3\,(R'_r/s)\,I'^{\,2}_r$. Šta treba da zaključiš: uz zanemaren $R_s$ i granu magnećenja motor je po fazi obično redno RL kolo u kome jedino element $R'_r/s$ „zna" za brzinu obrtanja — na njemu sede zajedno i toplota rotora i mehanička snaga, i baš iz tog kola slede sve formule za moment u ovom zadatku.
...
Ceo ovaj tok najbolje se vidi na sledećoj slici: na njoj je nacrtana kompletna momentna kriva $M_m(n)$ našeg motora po Klosovom obrascu, ali u funkciji **brzine** umesto klizanja (veza je $n = (1-s)\, n_s$, pa polazak $s=1$ odgovara levom kraju $n=0$, a sinhronizam desnom kraju), zajedno sa horizontalom momenta tereta.
...
> **Kako čitati sliku 48.2:** Na horizontalnoj osi je brzina obrtanja $n$ u $\mathrm{min^{-1}}$, od $0$ (polazak, $s=1$) do sinhrone $n_s = 1500\ \mathrm{min^{-1}}$ ($s=0$); na vertikalnoj je moment $M$ u $\mathrm{Nm}$, skala do $70\ \mathrm{Nm}$. Crvena kriva $M_m(n)$ je momentna karakteristika po uprošćenom Klosovom obrascu sa izračunatim parametrima $M_{\mathrm{kr}} = 68{,}97\ \mathrm{Nm}$ i $s_{\mathrm{kr}} = 0{,}2$: kreće od polaznog momenta $M_{\mathrm{pol}} = 26{,}5\ \mathrm{Nm}$ pri $n=0$, penje se do vrha $\approx 69\ \mathrm{Nm}$ na $n_{\mathrm{kr}} = 1200\ \mathrm{min^{-1}}$, pa strmo pada na nulu u $1500\ \mathrm{min^{-1}}$. Siva horizontala je konstantni moment tereta $M_T = \mathrm{konst.} = 20\ \mathrm{Nm}$; crna tačka na strmoj (stabilnoj) desnoj grani jedini je presek u prikazanom opsegu — radna tačka — a crvena strelica nadole od nje vodi do očitane brzine $n = 1455{,}6\ \mathrm{min^{-1}}$ (drugo, matematičko rešenje $s_{T2} = 1{,}349$ odgovaralo bi negativnoj brzini i van je slike). Iznad samog grafika na slici je ostao i red izvođenja $M_{\mathrm{pol}} = M_{\mathrm{nom}}\, s_{\mathrm{nom}} \left(I_{\mathrm{pol}}/I_{\mathrm{nom}}\right)^2 = 26{,}526\ \mathrm{Nm}$ — to je formula iz Koraka 3, preneta iz originalne zbirke, a ne deo dijagrama. Šta treba da zaključiš: motor ubrzava dokle god je crvena kriva iznad sive horizontale, a ustali se tamo gde se one seku na opadajućoj (stabilnoj) grani — ovde na $1455{,}6\ \mathrm{min^{-1}}$, malo iznad nazivne brzine, jer je teret manji od nazivnog momenta.
...
Sledeća slika prikazuje obe momentne karakteristike — za spregu trougao i za spregu zvezda — pri istom mrežnom (linijskom) naponu na priključnim stezaljkama.
...
> **Kako čitati sliku 48.3:** Ose su iste kao na Slici 48.2: brzina $n$ u $\mathrm{min^{-1}}$ ($0$–$1500$) na horizontalnoj i moment $M$ u $\mathrm{Nm}$ (do $70\ \mathrm{Nm}$) na vertikalnoj osi. Zelena kriva, na slici označena natpisom „Trougao", jeste karakteristika sa Slike 48.2: polazni moment $26{,}5\ \mathrm{Nm}$ pri $n=0$, vrh $\approx 69\ \mathrm{Nm}$ na $1200\ \mathrm{min^{-1}}$. Crvena kriva, označena „Zvezda", ista je ta kriva spljoštena tačno **tri puta** po visini (jer je $M \sim U_f^{\,2}$, a fazni napon je u zvezdi $\sqrt{3}$ puta manji): polazni moment $8{,}842\ \mathrm{Nm}$, vrh $68{,}97/3 \approx 23\ \mathrm{Nm}$ — na istoj brzini $1200\ \mathrm{min^{-1}}$, jer $s_{\mathrm{kr}}$ ne zavisi od napona, pa se oblik i položaj vrha ne menjaju. Crni kružić sa brojem **1** na levom kraju crvene krive ($n = 0$) je polazna tačka pri polasku u zvezdi, a siva horizontala kroz nju, sa natpisom $M_T < M_{\mathrm{p}Y} = 8{,}842\ \mathrm{Nm}$, označava granični moment tereta: svaki teret ispod te linije zvezda-polazak savlađuje, svaki iznad — ne. Šta treba da zaključiš: upuštač zvezda–trougao kupuje tri puta manju polaznu struju po ceni tri puta manjeg momenta — teret od $20\ \mathrm{Nm}$ iz dela (a) leži iznad granične horizontale, pa se sa ovim upuštačem motor ne bi ni pokrenuo.
