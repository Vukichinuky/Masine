...
Prva slika prikazuje dodatni otpor i **moment** u funkciji brzine. Čitaj je ovako: horizontalna osa je brzina $n$ u $\mathrm{min^{-1}}$ (od $0$ do $1500$); leva spoljašnja osa je $R'_{rd}$ u $\Omega$, a leva unutrašnja moment $M$ u $\mathrm{Nm}$. Ružičasta prava je naš zakon $R'_{rd}(n)$: kreće od $3{,}026\ \Omega$ i linearno pada do nule kod $n_{pr} = 825\ \mathrm{min^{-1}}$, posle čega ostaje nula. Debela crvena kriva je moment: konstantnih $83{,}67\ \mathrm{Nm}$ od tačke **1** (polazak, $n=0$) do tačke **2** ($n_{pr} = 825\ \mathrm{min^{-1}}$ — izlazak na prirodnu karakteristiku), a zatim opada po prirodnoj karakteristici do tačke **3** (stacionarno stanje, $n_n = 1400\ \mathrm{min^{-1}}$, $M_n \approx 24{,}4\ \mathrm{Nm}$). Siva isprekidana kriva je cela prirodna momentna karakteristika (ona koju bi motor imao bez ikakvog otpornika — pri direktnom polasku dala bi samo oko $63\ \mathrm{Nm}$ u $n=0$): vidi se da je crvena putanja zapravo "vrh" svih pomerenih karakteristika sve do $825\ \mathrm{min^{-1}}$, a od tada se poklapa sa prirodnom.
...
Druga slika ima istu ružičastu pravu $R'_{rd}(n)$, ali unutrašnja leva osa sada prikazuje **struju statora** $I_{sf}$ u $\mathrm{A}$. Crvena linija je struja tokom zaleta: konstantnih $28{,}134\ \mathrm{A}$ od tačke **1** ($n = 0$) do tačke **2** ($n_{pr} = 825\ \mathrm{min^{-1}}$), a zatim opada po prirodnoj karakteristici do tačke **3** — nazivnih $5{,}8\ \mathrm{A}$ pri $1400\ \mathrm{min^{-1}}$. Siva isprekidana kriva je struja po prirodnoj karakteristici: pri direktnom polasku ona bi u $n=0$ iznosila oko $36\ \mathrm{A}$ — više nego sa otpornikom, i to uz manji polazni moment! U tome je dvostruka korist rotorskog otpornika.
