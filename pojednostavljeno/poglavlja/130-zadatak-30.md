...
Sledeća slika prikazuje kompletnu ekvivalentnu pofaznu šemu asinhrone mašine — jedno jedino električno kolo u kome su stator, grana magnećenja i (svedeni) rotor vezani zajedno.

![Ekvivalentna pofazna šema asinhrone mašine](../slike/fig-30.1.png)

**Slika 30.1 —** Ekvivalentna pofazna šema asinhrone mašine.

> **Kako čitati sliku 30.1:** Ovo je električna šema jedne faze: cik-cak simboli su otpornici (u $\mathrm{\Omega}$), namotani simboli kalemovi (reaktanse, takođe u $\mathrm{\Omega}$); crne linije su provodnici, a **crvene strelice označavaju usvojene referentne smerove** struja (u $\mathrm{A}$) i napona (u $\mathrm{V}$). Prati struju od ulaza: na levim priključcima je fazni napon $\overline{U}_{sf}$ (uspravna crvena strelica, znak $+$ gore); statorska struja $\overline{I}_{sf}$ ulazi gornjim provodnikom i prolazi redno kroz **otpornik $R_s$** (termogeni otpor statora; kod nas $0{,}239\ \mathrm{\Omega}$) i **kalem $X_{\gamma s}$** (reaktansa rasipanja statora; iz Koraka 1: $0{,}754\ \mathrm{\Omega}$). U čvoru iza njih struja se grana: deo $\overline{I}_{0f}$ (struja praznog hoda) silazi u **granu magnećenja** — paralelnu vezu otpornika $R_{Fe}$ (kroz njega teče $\overline{I}_{Fe}$, aktivna komponenta koja predstavlja gubitke u gvožđu) i kalema $X_\mu$ (kroz njega $\overline{I}_\mu$, struja magnećenja koja stvara zajednički fluks); crvena zagrada sa oznakom $\overline{E}_{sf}$ i znakom $+$ pokazuje da na toj grani vlada indukovana elektromotorna sila. Ostatak, svedena rotorska struja $\overline{I}\,'_{rf}$, nastavlja gornjim provodnikom kroz **kalem $X'_{\gamma r}$** (svedena reaktansa rasipanja rotora; iz Koraka 5: $1{,}108\ \mathrm{\Omega}$) i **promenljivi otpornik $R'_r/s$** (cik-cak simbol sa kosom strelicom — vrednost mu zavisi od klizanja: pri $s = 1$ jednak je svedenom $R'_r = 0{,}374\ \mathrm{\Omega}$ iz Koraka 3, a pri malom klizanju naraste višestruko), pa se donjim provodnikom vraća ka izvoru. Šta treba da zaključiš: rotorska grana sme ovako, galvanski, da se nadoveže na statorsku samo zato što su $X'_{\gamma r}$ i $R'_r$ prethodno **svedeni** na statorski naponski nivo — upravo to svođenje je posao ovog zadatka — a sva zavisnost od opterećenja sedi u jednom jedinom elementu, $R'_r/s$.
...
Sledeća slika prikazuje tu približnu šemu.

![Približna ekvivalentna šema asinhrone mašine](../slike/fig-30.2.png)

**Slika 30.2 —** Približna ekvivalentna šema asinhrone mašine.

> **Kako čitati sliku 30.2:** Ista simbolika kao na slici 30.1 (cik-cak = otpornik, namotaj = reaktansa, crveno = referentni smerovi i napomene), ali je topologija promenjena: odmah iza levih priključaka (napon $\overline{U}_{sf}$, ulazna struja $\overline{I}_{sf}$) kolo se deli na **dve nezavisne paralelne grane**. Gornja, radna grana je prosta redna veza $R_{sf}$, $X_{\gamma s}$, $X'_{\gamma r}$ i promenljivog otpornika $R'_r/s$ (kosa strelica: zavisi od klizanja) i njome teče svedena rotorska struja $\overline{I}\,'_{rf}$. Donja grana vodi kroz **svoju kopiju** elemenata $R_{sf}$ i $X_{\gamma s}$ do grane magnećenja — paralelne veze $R_{Fe}$ (struja $\overline{I}_{Fe}$) i $X_\mu$ (struja $\overline{I}_\mu$) — i njome teče struja praznog hoda $\overline{I}_{0f}$. Obrati pažnju da se $R_{sf}$ i $X_{\gamma s}$ na slici pojavljuju **dva puta**, po jednom u svakoj grani: to je cena "rastavljanja" kola na dva nezavisna, opravdana time što je pad napona na njima mali u poređenju sa naponom mreže. Dve uokvirene crvene napomene nose ključne pretpostavke: $I_0 \approx \mathrm{const}$ (struju praznog hoda diktira napon mreže, pa se sa opterećenjem gotovo ne menja) i $\overline{I}_{sf} = \overline{I}\,'_{rf} + \overline{I}_{0f}$ (ukupna statorska struja je fazorski zbir struja dveju grana). Šta treba da zaključiš: približna šema pretvara spregnuto kolo u dva prosta kola koja se rešavaju odvojeno pa saberu — ali i ona, kao i tačna šema sa slike 30.1, radi isključivo sa **svedenim** rotorskim parametrima $X'_{\gamma r}$ i $R'_r$ koje u ovom zadatku računamo.
