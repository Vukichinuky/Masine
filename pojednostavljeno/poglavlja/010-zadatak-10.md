...
Sledeća slika prikazuje vektorski dijagram sinhronog motora u potpobuđenom režimu; na njoj se vidi i ključna geometrijska jednakost koju ćemo odmah zatim izvesti.
...
> **Kako čitati sliku 10.1:** Ovo je fazorski dijagram jedne faze (teorijska slika, bez brojeva): dužina svake strelice je efektivna vrednost (naponi u $\mathrm{V}$, struja u $\mathrm{A}$, u svojoj razmeri), a ugao je fazni stav. Svi fazori polaze iz zajedničkog početka na dnu slike; zamišljena rotacija je suprotna kazaljci na satu, pa fazor zakrenut udesno od nekog drugog za njim *kasni*. Referentni fazor je napon $\mathbf{U}_{\mathrm{f}}$, nacrtan vertikalno naviše. Struja $\mathbf{I}_{\mathrm{f}}$ je zakrenuta udesno za ugao $\varphi$ (donji luk) — kasni za naponom, jer potpobuđen motor uzima reaktivnu snagu iz mreže. Elektromotorna sila $\mathbf{E}_{0\mathrm{f}}$ leži između njih, zakrenuta za ugao opterećenja $\delta$ (gornji luk kod početka). Kratki fazor $jX_{\mathrm{s}}\mathbf{I}_{\mathrm{f}}$ (gore desno) spaja vrh $\mathbf{E}_{0\mathrm{f}}$ sa vrhom $\mathbf{U}_{\mathrm{f}}$ i time grafički "zatvara" jednačinu $\overline{U}_{\mathrm{f}} = \overline{E}_{0\mathrm{f}} + jX_{\mathrm{s}}\overline{I}_{\mathrm{f}}$; isprekidane pomoćne linije sa oznakom pravog ugla (kvadratić sa tačkom) pokazuju da je on **normalan na struju** (množenje sa $j$ = rotacija za $90°$), pa sa horizontalom zaklapa isti ugao $\varphi$ koji je kotiran i uz njegov vrh. Isprekidana horizontalna kota sa natpisom $E_{0\mathrm{f}}\sin\delta = X_{\mathrm{s}}I_{\mathrm{f}}\cos\varphi$ je srce slike: horizontalna projekcija fazora $\mathbf{E}_{0\mathrm{f}}$ (leva strana jednakosti) i horizontalna projekcija fazora $jX_{\mathrm{s}}\mathbf{I}_{\mathrm{f}}$ (desna strana) moraju biti jednake po veličini, jer sam $\mathbf{U}_{\mathrm{f}}$ horizontalnu projekciju nema. **Šta treba da zaključiš:** iz čiste geometrije dijagrama sledi $I_{\mathrm{f}}\cos\varphi = \dfrac{E_{0\mathrm{f}}}{X_{\mathrm{s}}}\sin\delta$ — jednakost iz koje u nastavku izrasta cela ugaona karakteristika snage i momenta.
...
Sledeća slika prikazuje tu zavisnost grafički.
...
> **Kako čitati sliku 10.2:** Na horizontalnoj osi je ugao opterećenja $\delta$ u radijanima, sa obeleženim vrednostima $0$, $\pi/2$ i $\pi$; na vertikalnoj osi su aktivna snaga $P$ (u $\mathrm{W}$) i moment $M$ (u $\mathrm{Nm}$) — jedna ista sinusna kriva važi za obe veličine, jer se razlikuju samo za konstantan faktor $\omega_{\mathrm{sm}}$. Puna crna kriva je polutalas sinusoide $M = M_{\mathrm{max}}\sin\delta$: raste od nule (za $\delta = 0$, neopterećena mašina), dostiže teme, pa se spušta nazad na nulu u $\delta = \pi$. Teme je obeleženo tačkom i isprekidanim linijama do obe ose: leži tačno na $\delta = \pi/2$, a njegova visina je $P_{\mathrm{max}}$ odnosno $M_{\mathrm{max}}$ — za naš motor $M_{\mathrm{max}} = 114{,}87\ \mathrm{kNm}$, tj. $P_{\mathrm{max}} = 12{,}03\ \mathrm{MW}$. Nazivna radna tačka bila bi niže na levoj, rastućoj grani: momentu $M_{\mathrm{n}} = 38{,}2\ \mathrm{kNm}$ odgovara $\sin\delta_{\mathrm{n}} = 38{,}2/114{,}87 = 0{,}33$, tj. $\delta_{\mathrm{n}} \approx 19°$. Leva grana ($\delta < \pi/2$) je **stabilna** — porast tereta povećava i $\delta$ i moment, pa se nađe nova ravnoteža; desna grana ($\delta > \pi/2$) je **nestabilna** — dalji porast $\delta$ moment smanjuje i mašina ispada iz sinhronizma. **Šta treba da zaključiš:** maksimum snage i momenta je tačno na $\delta = \pi/2$ i to je granica statičke stabilnosti — upravo tu vrednost računamo u Koraku 5.
...
Sledeća slika prikazuje vektorski dijagram napona za nazivni režim, u kome je motor natpobuđen (struja prednjači naponu — uobičajen način vođenja sinhronog motora, mini-lekcija 4).
...
> **Kako čitati sliku 10.3:** Referentni fazor je napon $\mathbf{U}_{\mathrm{fn}}$, nacrtan vertikalno naviše; dužine su efektivne vrednosti u $\mathrm{V}$ (struja u $\mathrm{A}$, u svojoj razmeri), a pošto se fazori obrću suprotno kazaljci, fazor zakrenut ulevo od $\mathbf{U}_{\mathrm{fn}}$ *prednjači*, a udesno *kasni*. Struja $\mathbf{I}_{\mathrm{fn}}$ je na levoj strani, pod uglom $\varphi_{\mathrm{n}}$ prema naponu ($\cos\varphi_{\mathrm{n}} = 0{,}8$, tj. $\varphi_{\mathrm{n}} \approx 36{,}9°$) — prednjači, što je otisak natpobuđenog režima. Elektromotorna sila $\mathbf{E}_{0\mathrm{fn}}$ je na desnoj strani, duža od napona ($E_{0\mathrm{fn}} = 9154{,}6\ \mathrm{V}$ prema $U_{\mathrm{fn}} = 6350{,}9\ \mathrm{V}$) i zakrenuta za ugao opterećenja $\delta_{\mathrm{n}}$ (donji luk; iz $\sin\delta_{\mathrm{n}} = X_{\mathrm{s}}I_{\mathrm{fn}}\cos\varphi_{\mathrm{n}}/E_{0\mathrm{fn}} = 3804{,}8 \cdot 0{,}8/9154{,}6$ sledi $\delta_{\mathrm{n}} \approx 19{,}4°$). Fazor $jX_{\mathrm{s}}\mathbf{I}_{\mathrm{fn}}$ (dužine $X_{\mathrm{s}}I_{\mathrm{fn}} = 3804{,}8\ \mathrm{V}$) ide od vrha $\mathbf{E}_{0\mathrm{fn}}$ ka vrhu $\mathbf{U}_{\mathrm{fn}}$ i zatvara jednačinu $\overline{U}_{\mathrm{fn}} = \overline{E}_{0\mathrm{fn}} + jX_{\mathrm{s}}\overline{I}_{\mathrm{fn}}$; isprekidane pomoćne linije sa oznakama pravog ugla (kvadratić sa tačkom) potvrđuju da je normalan na struju. Zakrivljena strelica kotira ugao $\pi/2 + \varphi_{\mathrm{n}}$ — ugao između pravca stranice $U_{\mathrm{fn}}$ i pravca stranice $X_{\mathrm{s}}I_{\mathrm{fn}}$ u uočenom trouglu ($90°$ od množenja sa $j$ plus $\varphi_{\mathrm{n}}$ od zakretanja struje); mali ugao $\varphi_{\mathrm{n}}$ ucrtan pri vrhu slike ista je veličina, prenesena pomoćnom konstrukcijom. **Šta treba da zaključiš:** trougao sa stranicama $U_{\mathrm{fn}} = 6350{,}9\ \mathrm{V}$ i $X_{\mathrm{s}}I_{\mathrm{fn}} = 3804{,}8\ \mathrm{V}$ koje zaklapaju tup ugao $\pi/2+\varphi_{\mathrm{n}}$ jednoznačno određuje treću stranicu $E_{0\mathrm{fn}} = 9154{,}6\ \mathrm{V}$ — upravo nju u nastavku računamo kosinusnom teoremom.
...
> **Kako čitati sliku 10.4:** Dijagram je sada pravougli trougao. Kateta $\mathbf{U}_{\mathrm{fn}}$ je vertikalna (dužine $6350{,}9\ \mathrm{V}$), kateta $\mathbf{E}_{0\mathrm{fn}}$ horizontalna (dužine $9154{,}6\ \mathrm{V}$ — ista kao u nazivnom režimu, jer pobudu nismo menjali), a veliki luk u koordinatnom početku kotira ugao između njih: $\delta_{\mathrm{max}} = \pi/2$. Hipotenuza $jX_{\mathrm{s}}\mathbf{I}_{\mathrm{f1}}$ spaja vrh $\mathbf{E}_{0\mathrm{fn}}$ (dole desno) sa vrhom $\mathbf{U}_{\mathrm{fn}}$ (gore levo); njena dužina je $X_{\mathrm{s}}I_{\mathrm{f1}} = \sqrt{E_{0\mathrm{fn}}^2 + U_{\mathrm{fn}}^2} = 11\,141{,}8\ \mathrm{V}$, odakle je $I_{\mathrm{f1}} = 768{,}4\ \mathrm{A}$. Struja $\mathbf{I}_{\mathrm{f1}}$ polazi iz koordinatnog početka, **normalna je na hipotenuzu** (pad $jX_{\mathrm{s}}\mathbf{I}_{\mathrm{f1}}$ uvek prednjači struji za $90°$) i sa naponom zaklapa ugao $\varphi_1$ (manji luk): $\cos\varphi_1 = E_{0\mathrm{fn}}/(X_{\mathrm{s}}I_{\mathrm{f1}}) = 0{,}822$, tj. $\varphi_1 \approx 34{,}7°$. Primeti da je struja sada nacrtana **desno** od napona — kasni za njim: pri $\delta = \pi/2$ motor, iako mu je pobuda i dalje nazivna ("natpobuđena"), uzima reaktivnu snagu iz mreže. **Šta treba da zaključiš:** pri maksimalnom momentu fazori $\mathbf{U}_{\mathrm{fn}}$ i $\mathbf{E}_{0\mathrm{fn}}$ stoje pod pravim uglom, pa struju daje obična Pitagorina teorema — $I_{\mathrm{f1}} = 768{,}4\ \mathrm{A}$, skoro tri puta ($2{,}93\times$) više od nazivne.
