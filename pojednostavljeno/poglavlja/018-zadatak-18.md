...
Slika 18.1 prikazuje oba režima u istom koordinatnom sistemu. Kako je čitati: fazor napona $\underline{U}_f$ je nacrtan vertikalno naviše (referentni pravac, ugao $0^\circ$). Levo od njega, pod uglovima $\delta_{\mathrm{n}} = 27{,}9^\circ$ i $\delta_1 = 29{,}5^\circ$, stoje fazori EMS $\underline{E}_{0f\mathrm{n}}$ (pre) i $\underline{E}_{0f1}$ (posle, malo kraći i malo više zakrenut). Desno dole od $\underline{U}_f$, pod uglovima $\varphi_{\mathrm{n}} = 36{,}87^\circ$ i $\varphi_1 = 32{,}2^\circ$, stoje fazori struja $\underline{I}_{f\mathrm{n}}$ (pre) i $\underline{I}_{f1}$ (posle, kraći i bliži naponu). Isprekidane linije su pomoćne konstrukcije: vrh svakog fazora EMS dobija se sabiranjem $\underline{U}_f$ i pada $\mathrm{j}X_{\mathrm{S}}\underline{I}_f$ (koji je normalan na odgovarajuću struju).
...
Šta dijagram "precizno obuhvata" (to je i traženo u postavci), po pravilima iz mini-lekcije 8:

- Vrhovi struja $\underline{I}_{f1}$ i $\underline{I}_{f\mathrm{n}}$, pre i posle smanjenja fluksa, leže **na istoj horizontali**, jer aktivna snaga ostaje nepromenjena, a važi $P = 3\,U_f I_f\cos\varphi$ — dakle $I_f\cos\varphi = \mathrm{const.}$, a to je upravo vertikalna udaljenost vrha fazora struje od koordinatnog početka (na slici označeno donjom isprekidanom horizontalom).
- Slično, vrhovi $\underline{E}_{0f1}$ i $\underline{E}_{0f\mathrm{n}}$ leže **na istoj vertikali** (leva isprekidana vertikala, uz koju stoji gornja horizontalna zagrada), jer iz $P = \mathrm{const.}$ sledi $E_{0f}\sin\delta = \mathrm{const.}$, a to je upravo horizontalna udaljenost vrha EMS od pravca $\underline{U}_f$.
- Horizontalni razmak između vrhova struja (označen sa $Q\sim I_f\sin\varphi$) pokazuje **smanjenje reaktivne snage**: nova struja ima manju horizontalnu komponentu, tj. generator posle intervencije daje manje reaktivne snage mreži.
