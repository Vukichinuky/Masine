...
Sledeća slika prikazuje obe sprege istog motora. Čitaj je ovako: gore je veza trougao — svaki namotaj (spirala) vezan je direktno između dva mrežna priključka $U$, $V$, $W$, pa "vidi" punih $380\ \mathrm{V}$; dole levo je veza zvezda — namotaji se stiču u zvezdište, svaki vidi $U_{sf} = 220\ \mathrm{V}$, a linijska struja je tri puta manja nego u trouglu ($\overline{I}_{sY} = \overline{I}_{s\Delta}/3$). Dole desno je ista veza zvezda predstavljena preko **ekvivalentne zvezde** (isprekidani cik-cak simboli, isti oni koji su ucrtani i unutar trougla na gornjoj šemi): ako se sprega trougao zameni svojom ekvivalentnom zvezdom (fazni napon $220\ \mathrm{V}$, tri puta manja impedansa po fazi), onda se stvarna veza zvezda, posmatrana kroz tu istu ekvivalentnu impedansu, ponaša kao da je fazni napon svega $220/3\ \mathrm{V}$ — što je slikovit dokaz da je linijska struja u zvezdi **tri puta** manja nego u trouglu.
...
Sledeća slika prikazuje obe karakteristike i put radne tačke pri upuštanju. Čitaj je ovako: donja puna kriva je karakteristika u **zvezdi**, gornja isprekidana u **trouglu** (tri puta viša); crvenom linijom je ucrtan put radne tačke: motor kreće iz tačke **1** (polazak u zvezdi, $n = 0$), zaleće se duž krive zvezde do tačke **2** (brzina prebacivanja, oko $1160\ \mathrm{min}^{-1}$ — nju računamo u ovom zadatku), tu se sprega preklopi pa radna tačka **vertikalno skoči** u tačku **3** na karakteristici trougla (brzina se u trenutku preklopa ne menja, a moment skoči tri puta), i na kraju se motor duž krive trougla zaleti do konačne radne brzine blizu $n_s = 1500\ \mathrm{min}^{-1}$.
