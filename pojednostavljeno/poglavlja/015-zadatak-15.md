...
Pre računa, pogledajmo vektorski dijagram generatora u ovom režimu; detaljno čitanje dato je odmah ispod slike.
...
> **Kako čitati sliku 15.1:** Referentni pravac je horizontala na kojoj leže plavi fazor struje $\overline{I}_{sf}$ i zeleni fazor napona $\overline{U}_{sf}$ — u fazi su, jer je $\cos\varphi = 1$ (u brojevima: $I_{sf} = 115{,}5\ \mathrm{A}$, $U_{sf} \approx 220\ \mathrm{V}$; strelica struje je kraća čisto grafički — struja i napon su različite fizičke veličine, pa se ne porede po dužini). Rotacija fazora je suprotna kazaljci na satu, pa fazor iznad ose **prednjači**. Iz vrha $\overline{U}_{sf}$ diže se vertikalno zeleni fazor pada napona $j \cdot \overline{I}_{sf} \cdot X_s$, dužine $I_{sf} X_s = 138{,}6\ \mathrm{V}$ — upravan na struju, jer množenje sa $j$ zakreće fazor za $90^{\circ}$ unapred. Njihov zbir je hipotenuza $\overline{E}_{0sf} \approx 260\ \mathrm{V}$ (zelena, gore desno), koja prednjači naponu za ugao opterećenja $\delta = 32{,}2^{\circ}$, označen lukom pri korenu — kod generatora EMS uvek prednjači naponu. Crveni fazor $\overline{F}_r$ (gore levo) je magnetopobudna sila rotora: prednjači EMS za $\pi/2$ (označeno na slici), jer indukovana EMS kasni $90^{\circ}$ za fluksom koji je stvara. Ceo dijagram je pravougli trougao: nalegla kateta $U_{sf} \approx 220\ \mathrm{V}$, naspramna kateta $I_{sf} X_s = 138{,}6\ \mathrm{V}$, hipotenuza $E_{0sf} \approx 260\ \mathrm{V}$ — iz njega direktno slede veze $E_{0sf}\sin\delta = I_{sf}X_s$ i $\tan\delta = I_{sf}X_s/U_{sf}$ koje nose ceo račun (Korak 2). **Šta treba da zaključiš:** pri $\cos\varphi = 1$ geometrija se svodi na pravougli trougao, pa se $U_{sf}$ i $E_{0sf}$ — koje zadatak ne zadaje — čistom trigonometrijom izražavaju preko zadatih $I_{sf}$, $X_s$ i $\delta$.
