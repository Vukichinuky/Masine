...
Slika 23.1 objedinjuje sva razmatranja: prikazuje $p$–$q$ ravan ($q$-osa horizontalno, $p$-osa vertikalno, obe isprekidane) sa sve četiri granice i sa fazorskim trouglom $O O_1 A$ nazivnog režima ucrtanim unutar karte. Kako da je čitaš: velika kružnica kroz $A$ sa centrom u koordinatnom početku $O_1$ je statorska granica (poluprečnika $1$); luk na desnoj strani, sa centrom u $O$, je rotorska granica (poluprečnika $1{,}379$); horizontalna prava kroz vrh je granica turbine $p = 0{,}9$; kosa prava iz gornjeg levog ugla, obeležena $\delta = 80{,}8^\circ$, je praktična granica stabilnosti kroz tačku $O$. Fazori (podvučene oznake): $u\underline{u}/x_{\mathrm{s}}$ od $O$ do $O_1$, $\mathrm{j}u\underline{i}_{\mathrm{n}}$ od $O_1$ do nazivne tačke $A$ (pod uglom $\varphi_{\mathrm{n}}$ prema $p$-osi) i $u\underline{e}_{0\mathrm{n}}/x_{\mathrm{s}}$ od $O$ do $A$.
