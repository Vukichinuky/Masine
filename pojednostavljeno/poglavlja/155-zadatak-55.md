...
> **Kako čitati sliku 55.1:** Ekvivalentna šema po jednoj fazi — prati struju od izvora ka rotoru. Skroz levo je crveni kružić sa strelicom naviše: **idealan strujni izvor**, model frekventnog pretvarača, koji kolu nameće faznu struju $\bar{I}_{sf}$ (pri polasku: strujni limit od $25\ \mathrm{A}$) i njenu učestanost $f_s$. Gornjim provodnikom struja $\bar{I}_{sf}$ (crvena strelica udesno) stiže do čvora u kome se deli na dve **paralelne** grane. Prva (srednja) grana je grana magnećenja: kalem sa oznakama $L_m$ i $X_m$ — induktivnost $L_m \approx 77{,}6\ \mathrm{mH}$ je konstanta mašine, a reaktansa $X_m = 2\pi f_s L_m$ raste sa učestanošću (pri $50\ \mathrm{Hz}$ iznosi $24{,}38\ \mathrm{\Omega}$, pri optimalnoj polaznoj učestanosti $1{,}23\ \mathrm{Hz}$ svega $0{,}6\ \mathrm{\Omega}$); kroz nju teče struja magnećenja $\bar{I}_\mu$ (crvena strelica nadole) koja stvara fluks mašine. Druga (desna) grana je rotor: otpornik nacrtan sa kosom strelicom — promenljiv, jer mu vrednost $R'_r/s$ zavisi od klizanja; pri polasku je $s = 1$, pa iznosi tačno $R'_r = 0{,}6\ \mathrm{\Omega}$. Kroz njega teče svedena rotorska struja $\bar{I}'_{rf}$ (crvena strelica na gornjem desnom kraku) — jedina koja pravi moment; pri usaglašenim impedansama obe grane nose po $I_{sf}/\sqrt{2} = 17{,}68\ \mathrm{A}$. Obe grane vise između ista dva čvora, pa je na njima isti napon — crvena dvostrana strelica $\bar{U}_{sf}$ sa znakom „+" gore (pri optimalnom polasku svega $\approx 10{,}6\ \mathrm{V}$); donji provodnik zatvara konturu nazad ka izvoru. Redna impedansa statora ($R_s$, $X_{\gamma s}$) i rasipna reaktansa rotora namerno **nisu nacrtane** — obrazloženje je u listi uprošćenja odmah ispod. Šta treba da zaključiš: cela fizika zadatka je jedna podela nametnute struje između $X_m$ i $R'_r/s$ — moment je najveći kada se struja po modulu podeli popola, tj. kada spuštanjem učestanosti reaktansa $X_m$ padne tačno na vrednost $R'_r$.
